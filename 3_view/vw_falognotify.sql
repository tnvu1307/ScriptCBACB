SET DEFINE OFF;
CREATE OR REPLACE FORCE VIEW VW_FALOGNOTIFY
...
SELECT F.AUTOID, F.OBJNAME, A2.EN_CDCONTENT OBJDESC,F.KEYNAME, F.KEYVALUE, F.ACTION, F.STATUS,
     ...
     CASE WHEN F.OBJNAME IN ('CBFABANKPAYMENT_RESULT') THEN 'FA: HOME/COMPLETED TRANSACTION'
          ...
          WHEN F.OBJNAME IN ('CATRANSFERCONFIRM') THEN 'FA: CORPORATE ACTION/CA - RIGHT OPTION MOVEMENT/GV FA1000'
          WHEN F.OBJNAME IN ('CAALLOCATE','CAALLOCATE_SWITH','CAALLOCATE_RIGHT') THEN 'FA: CORPORATE ACTION/GV FA1001, FA1004, FA1005'
          WHEN F.OBJNAME IN ('CBFA_FACAMAST') THEN 'FA: CORPORATE ACTION/CA - CREATE EVENT/CREATE CORPORATE ACTION'
          WHEN F.OBJNAME IN ('OTCCONFIRM') THEN ''
          WHEN F.OBJNAME IN ('CAMAST') THEN 'FA: CORPORATE ACTION/CA - CREATE EVENT/GV FA1002'
