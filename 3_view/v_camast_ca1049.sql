SET DEFINE OFF;
CREATE OR REPLACE FORCE VIEW V_CAMAST_CA1049
(VALUE, AUTOID, SYMBOL, CAMASTID, CODEID, 
 ...
 FORMOFPAYMENT, INSTRUCTION, RIGHTTRANSDL, INSDEADLINE, DELTD)
AS 
SELECT MST.CAMASTID VALUE, MST.AUTOID,SYM.SYMBOL,
        ...
        MST.DEBITDATE,MST.FORMOFPAYMENT, MST.INSTRUCTION, MST.RIGHTTRANSDL, MST.INSDEADLINE, MST.deltd
FROM CAMAST MST, SBSECURITIES SYM, TLPROFILES MAKER, TLPROFILES APPRV, SBSECURITIES TOSYM,
(
    SELECT SCHD.CAMASTID,
    SUM(CASE WHEN SCHD.ISCI = 'Y' THEN SCHD.AMT ELSE 0 END) AMT,
    SUM(CASE WHEN SCHD.ISSE = 'Y' THEN SCHD.QTTY ELSE 0 END) QTTY,
    SUM(ROUND(MST1.PITRATE *
         (CASE WHEN (CASE WHEN SCHD.PITRATEMETHOD = '##' THEN MST1.PITRATEMETHOD ELSE SCHD.PITRATEMETHOD END) = 'SC' THEN 1 ELSE 0 END)
        *(CASE WHEN (SCHD.ISCI= 'Y' AND CF.VAT='Y') THEN (CASE WHEN MST1.CATYPE = '016' THEN SCHD.INTAMT ELSE SCHD.AMT END) ELSE 0 END)
        /100
    ))TAXAMT,
    SUM((CASE WHEN SCHD.ISCI= 'Y' THEN SCHD.AMT ELSE 0 END) - ROUND(MST1.PITRATE *
            (CASE WHEN (CASE WHEN SCHD.PITRATEMETHOD = '##' THEN MST1.PITRATEMETHOD ELSE SCHD.PITRATEMETHOD END) = 'SC' THEN 0 ELSE 1 END)
           *(CASE WHEN (SCHD.ISCI= 'Y' AND CF.VAT='Y') THEN (CASE WHEN  MST1.CATYPE = '016' THEN SCHD.INTAMT ELSE SCHD.AMT END) ELSE 0 END)
           /100
    ))AMTAFTER
...
(
    ...
    FROM CASCHD
    WHERE DELTD='N' AND ISEXEC <> 'N'
    GROUP BY CAMASTID
) SCHD2,
(
    SELECT SUM(SE.TRADE + SE.MARGIN + SE.BLOCKWITHDRAW + SE.EMKQTTY + SE.BLOCKDTOCLOSE + SE.WTRADE + SE.MORTAGE + SE.BLOCKED + SE.SECURED + SE.REPO + SE.NETTING + SE.DTOCLOSE + SE.WITHDRAW + SE.HOLD) TRADE,
           NVL(SB.REFCODEID, SB.CODEID) CODEID
    FROM SEMAST SE, SBSECURITIES SB
    WHERE SE.CODEID = SB.CODEID
    GROUP BY NVL(SB.REFCODEID, SB.CODEID)
) SE,
(
