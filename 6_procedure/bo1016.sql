SET DEFINE OFF;
CREATE OR REPLACE PROCEDURE bo1016 (
...
-- BO1016: Phieu lenh ban TP/TPKB(Sell on bond/T-bill order) (Repos trading)
...
OPEN PV_REFCURSOR FOR
    ...
    select od.txdate, od.txtime, od.orderid, cf.custodycd, cf.fullname, af.acctno afacctno, sb.symbol,
        ...
        od.orderqtty, od.orderprice quoteprice, od.orderqtty*od.orderprice orderamt, od.orderprice/1000 GiaYet, tlp.tlname, tlp.tlfullname,
...
EXCEPTION
   WHEN OTHERS
   THEN
    plog.error('BO1016.' || sqlerrm || '.At:' || dbms_utility.format_error_backtrace);
      RETURN;
END;
/
