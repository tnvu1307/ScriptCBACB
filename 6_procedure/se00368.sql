SET DEFINE OFF;
CREATE OR REPLACE PROCEDURE se00368 (
 ...
 FOR
    SELECT * FROM (
        ...
        FROM (
            ...
            '005',CA.DEVIDENTSHARES,'010', CA.DEVIDENTRATE||'%','022', CA.DEVIDENTSHARES,'023',CA.EXRATE,'015',ca.interestrate||'%' , '016',ca.interestrate||'%', '1/1')) CA_TYLE,
            NVL(SUM(CAS.TRADE),0)  REPORT_QTTY,
            NVL(SUM(CAS.CUTPBALANCE),0) CUTPBALANCE,
            CASE
                WHEN  max(CA.CATYPE) = '015' OR  max(CA.CATYPE) = '016' OR  max(CA.CATYPE) = '005' OR  max(CA.CATYPE) = '006' OR  max(CA.CATYPE) = '022'  THEN
                        NVL(SUM(CAS.AMT + cas.cutamt + cas.sendamt ),0)
                ELSE    NVL(SUM(cas.pbalance + cas.balance + cas.cutpbalance),0)
             END  QTTY,
...
EXCEPTION
  WHEN OTHERS
   THEN
   DBMS_OUTPUT.PUT_LINE('SE00368 ERROR');
