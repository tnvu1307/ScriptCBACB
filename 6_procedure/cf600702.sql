SET DEFINE OFF;
CREATE OR REPLACE PROCEDURE cf600702 (
...
OPEN PV_REFCURSOR FOR
    SELECT MST2.GC, MST2.SYMBOL, MST2.OTS, MST2.CLIENTNAME, MST2.QUANTITY,
    ...
    FROM(
        ...
        (
            ...
            (
                ...
                (
                    SELECT SB.ISINCODE, SB.ISSUERID, SB.CODEID, SB.SYMBOL,SB1.CODEID REFCODEID, SB1.SYMBOL REFSYMBOL,(CASE WHEN SB.REFCODEID IS NULL THEN SB.TRADEPLACE ELSE SB1.TRADEPLACE END) TRADEPLACE
        ...
        SELECT (CASE WHEN V_LCB = '%' THEN 'ALL' ELSE v_shortname END) GC,
               (CASE WHEN V_LCB = '%' THEN 'ALL' ELSE v_fullname END) CLIENTNAME,
               NULL SYMBOL, NULL OTS, 0 QUANTITY, 0 PREV_QUANTITY, 0 RATEOW, 0 PREV_RATEOW, 0 DELTACHANGE
        FROM DUAL
        WHERE NOT EXISTS (
            SELECT 1
            FROM CFMAST CF, AFMAST AF, FAMEMBERS FA, FAMEMBERS FA1, (
...
EXCEPTION
  WHEN OTHERS
   THEN
      plog.error ('CF600702: ' || SQLERRM || dbms_utility.format_error_backtrace);
      Return;
End;
/
