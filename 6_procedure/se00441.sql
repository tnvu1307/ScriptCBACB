SET DEFINE OFF;
CREATE OR REPLACE PROCEDURE se00441 (
...
-- GET REPORT'S DATA
 ...
 FOR
        SELECT MAX(CF.CUSTODYCD) CUSTODYCD, MAX(REPLACE(SB.SYMBOL,'_WFT',''))  SYMBOL, CA.REPORTDATE,
               ...
               '005',CA.DEVIDENTSHARES,'010', CA.DEVIDENTRATE,'023',CA.EXRATE, '1/1')) CA_TYLE, CA.EXPRICE EXPRICE, CA.CIROUNDTYPE CIROUNDTYPE,
        ...
        WHERE CF.CUSTODYCD = V_CUSTODYCD AND CF.CUSTID = AF.CUSTID
                ...
                AND CAS.STATUS ='O'
                AND CAS.DELTD <> 'Y'
        GROUP BY CA.CAMASTID, CA.REPORTDATE, CA.EXPRICE,CA.CIROUNDTYPE,SB.TRADEPLACE,SB.MARKETTYPE,SB.SECTYPE

        UNION ALL

        SELECT V_CUSTODYCD  CUSTODYCD, 'XXX' SYMBOL, TO_DATE(F_DATE,'DD/MM/YYYY') REPORTDATE,
                ' ' CA_TYLE, 0 EXPRICE,0 CIROUNDTYPE,
                0  REPORT_QTTY,
...
EXCEPTION
   WHEN OTHERS
   THEN
   plog.error('SE00441: Error: ' || SQLERRM || dbms_utility.format_error_backtrace);
      RETURN;
END;
