SET DEFINE OFF;
CREATE OR REPLACE PROCEDURE od6008_amc(
        ...
        FROM (
            ...
            FROM (
                ...
                (
                    SELECT CF.*
                    FROM CFMAST CF
                    LEFT JOIN (SELECT * FROM FAMEMBERS WHERE ROLES ='AMC') FA ON CF.AMCID= FA.AUTOID,(SELECT *FROM EMAILREPORT WHERE DELTD <> 'Y') E
                ...
                GROUP BY CF.FULLNAME, CF.CIFID, CF.CUSTID, SB.SYMBOL, SB.ISINCODE, ISS.FULLNAME, SE.CODEID, SE.ACCTNO, SE.NETTING, SE.BLOCKWITHDRAW, SE.WITHDRAW, SE.HOLD, SE.RECEIVING, SE.BLOCKED, SE.EMKQTTY, SE.TRADE, SE.MORTAGE, SB.SECTYPE, SE.TRADE1404
            ) SE
            LEFT JOIN (
