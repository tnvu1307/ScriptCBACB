SET DEFINE OFF;
CREATE OR REPLACE PROCEDURE se00171 (
 ...
 FOR
    ...
    FROM (
            SELECT MAX(CF.CUSTODYCD) CUSTODYCD, MAX(REPLACE(SB.SYMBOL,'_WFT',''))  SYMBOL, CA.REPORTDATE,
                   ...
                   MAX(CAS.TRADE)  REPORT_QTTY, SUM(CAS.CUTPBALANCE) CUTPBALANCE, SUM(CAS.CUTAQTTY) CUTAQTTY,
                   --NVL(CAL.QTTY,0) QTTY,
            ...
            GROUP BY CA.CAMASTID, CA.REPORTDATE, CA.EXPRICE,CA.CIROUNDTYPE,SB.TRADEPLACE,
                     SB.MARKETTYPE,SB.SECTYPE
