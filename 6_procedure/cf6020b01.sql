SET DEFINE OFF;
CREATE OR REPLACE PROCEDURE CF6020B01(
                        ...
                        FROM ( --BANG TEMP TINH SO DU CHUNG KHOAN
                             ...
                             FROM (
                                SELECT SE.CUSTID
                                        ...
                                        ,SUM(CASE WHEN SB.TRADEPLACE = '003' AND SB.DEPOSITORY <> '001' AND SB.ISSEDEPOFEE = 'N' THEN SE.RECEIVING - NVL(ETF.CKCC_RECEIVING, 0) ELSE 0 END) TOTAL_REC
                                        ,NVL((
                                            CASE WHEN SB.TRADEPLACE = '003' AND SB.DEPOSITORY <> '001' AND SB.ISSEDEPOFEE = 'N' THEN (CASE WHEN NVL(SI.CLOSEPRICE,0) > 0 THEN SI.CLOSEPRICE ELSE SB.PARVALUE END)
                                                 WHEN SB.SECTYPE IN ('008','011','001','002') THEN (CASE WHEN NVL(SI.CLOSEPRICE,0) > 0 THEN SI.CLOSEPRICE ELSE SB.PARVALUE END)
                                                 ELSE SB.PARVALUE END
                                        ),0) MARKET_PRICE

                                FROM
                                (
                                    SELECT CUSTID, ACCTNO, CODEID, NETTING, WITHDRAW, BLOCKWITHDRAW, TRADE, HOLD, BLOCKED, MORTAGE, RECEIVING, 'Yes' VAULT_STORAGE
                                    FROM SEMAST

                                    --UNION ALL

                                    --SELECT AFACCTNO, ACCTNO, CODEID, 0 NETTING, 0 WITHDRAW, 0 BLOCKWITHDRAW, TRADE, 0 HOLD, 0 BLOCKED, 0 MORTAGE, 0 RECEIVING, 'No' VAULT_STORAGE
                                    --FROM SENOCUSTATCOM
                                ...
                                JOIN (
                                    --BANG TAM SB
                                    SELECT SB.CODEID, SB.SECTYPE, SB.SYMBOL, SB1.CODEID REFCODEID, SB1.SYMBOL REFSYMBOL, SB.EXPDATE, SB.PARVALUE, SC.SHORTCD CURRENCY,
                                        (CASE WHEN SB.REFCODEID IS NULL THEN SB.TRADEPLACE ELSE SB1.TRADEPLACE END) TRADEPLACE,
                                        (CASE WHEN SB.REFCODEID IS NULL THEN SB.DEPOSITORY ELSE SB1.DEPOSITORY END) DEPOSITORY,
                                        (CASE WHEN SB.REFCODEID IS NULL THEN SB.ISSEDEPOFEE ELSE SB1.ISSEDEPOFEE END) ISSEDEPOFEE
                                ...
                                LEFT JOIN (
                                    SELECT SE.ACCTNO SEACCTNO, OD.EXECTYPE, (CASE WHEN ETF.STATUS = 'A' THEN 0 ELSE DECODE(OD.EXECTYPE,'NB',SUM(ETF.QTTY),0) END) CKCC_RECEIVING
                                    FROM ETFWSAP ETF, ODMAST OD, SEMAST SE
                                    WHERE ETF.ORDERID = OD.ORDERID
                                    AND OD.ODTYPE = 'SWE'
                                    AND OD.DELTD <> 'Y'
                                    AND ETF.AFACCTNO = SE.AFACCTNO
                                    AND ETF.CODEID = SE.CODEID
                                    GROUP BY SE.ACCTNO, OD.EXECTYPE, ETF.STATUS
                                ) ETF ON ETF.SEACCTNO = SE.ACCTNO
                                WHERE SE.CUSTID IS NOT NULL
                                GROUP BY SE.CUSTID, SE.ACCTNO, SB.SECTYPE, SB.REFSYMBOL, SB.CURRENCY, SE.VAULT_STORAGE, SB.TRADEPLACE, SB.DEPOSITORY, SB.ISSEDEPOFEE,
...
EXCEPTION
  WHEN OTHERS
   THEN
      plog.error ('cf6020B01: ' || SQLERRM || dbms_utility.format_error_backtrace);
      Return;
