SET DEFINE OFF;
CREATE OR REPLACE PROCEDURE od6008b02(
...
SELECT CF.FULLNAME FUNDNAME, CF.CIFID, CF.CUSTID
        ...
        ,CF.MCUSTODYCD,(case when V_MCUSTODYCD = '%' then '' else CF.MFULLNAME end) MFULLNAME,(case when V_MCUSTODYCD = '%' then '' else CF.MCIFID2 end) MCIFID2
FROM
(
    SELECT * FROM DDMAST WHERE STATUS NOT IN ('C')
) DD,
(
    SELECT CF.*, CF1.FULLNAME MFULLNAME, CF1.MCIFID MCIFID2
    FROM CFMAST CF
...
EXCEPTION
  WHEN OTHERS
   THEN
      PLOG.ERROR ('OD600802: ' || SQLERRM || DBMS_UTILITY.FORMAT_ERROR_BACKTRACE);
      RETURN;
END;
/
