SET DEFINE OFF;
CREATE OR REPLACE PROCEDURE compile_pck_web
   IS
--
...
BEGIN
    ...
    (
        ...
        union all
        SELECT *FROM (SELECT OBJECT_TYPE,OBJECT_NAME FROM USER_OBJECTS WHERE object_type in ('PACKAGE BODY','PACKAGE') and object_name ='CSPKS_RMPROC' ORDER BY OBJECT_TYPE DESC)
        union all
        ...
        union all
        SELECT *FROM (  SELECT OBJECT_TYPE,OBJECT_NAME FROM USER_OBJECTS WHERE object_type in ('PACKAGE BODY','PACKAGE') and object_name ='FOPKS_SA' ORDER BY OBJECT_TYPE DESC)
        union all
        SELECT *FROM (  SELECT OBJECT_TYPE,OBJECT_NAME FROM USER_OBJECTS WHERE object_type in ('PACKAGE BODY','PACKAGE') and object_name ='FOPKS_TX' ORDER BY OBJECT_TYPE DESC)
    )
    LOOP
