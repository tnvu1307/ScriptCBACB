SET DEFINE OFF;
CREATE OR REPLACE PROCEDURE od6032b(
       ...
       SELECT *
        ...
        --==========================================MAIN SELECT QUERY=======================================================================
            ...
            FROM (
                        ...
                        FROM ( --BANG TEMP TINH SO DU CHUNG KHOAN
                             ...
                             FROM (
                                SELECT SE.CUSTID
                                        ...
                                        ,(CASE WHEN SB.REFSYMBOL IS NULL THEN SUM( SE.BLOCKED + SE.MORTAGE + SE.BLOCKTRANFER + SE.BLOCKDTOCLOSE + SE.BLOCKWITHDRAW + SE.EMKQTTY + SE.NETTING) ELSE 0 END) TOTAL_HLD
                                        ,(CASE WHEN SB.REFSYMBOL IS NULL THEN SUM(SE.TRADE + SE.HOLD + SE.TRANSFER+ SE.WITHDRAW) ELSE 0 END) TOTAL_AVL
                    ...
                    UNION ALL
                    ----
                    SELECT CUSTID,SYMBOL,ISINCODE,ISSUERID,SECURITIES_TYPE,SECURITIES_STATUS,SUM(QUANTITY) QUANTITY,
                    CURRENCY,MARKET_PRICE FROM(
                     select CF.CUSTID,CA.SYMBOL,CA.ISINCODE,SB.ISSUERID, (CASE WHEN SB.SECTYPE IN ('001','002','008') THEN 'EQUITY' --CO PHIEU/CHUNG CHI QUY
                                    WHEN SB.SECTYPE IN ('003','006') THEN 'BOND' --TRAI PHIEU
                                    WHEN SB.SECTYPE = '013' THEN 'CD' --CHUNG CHI TIEN GUI
                                    WHEN SB.SECTYPE = '011' THEN 'CW' --CHUNG QUYEN
                                    WHEN SB.SECTYPE = '008' THEN 'FUCE'
                                    WHEN SB.SECTYPE = '009' THEN 'TD'
                                    WHEN SB.SECTYPE = '012' THEN 'T-Bill'
                                    WHEN SB.SECTYPE = '015' THEN 'Warrant'
                                    ELSE '' END
                              ) SECURITIES_TYPE,'CER' SECURITIES_STATUS,CA.QTTY QUANTITY,SB.CURRENCY,
                       NVL((CASE WHEN SB.SECTYPE IN ('003','006') OR SB.TRADEPLACE IN ('003')  THEN SB.PARVALUE
                        ELSE SI.CLOSEPRICE END),0) MARKET_PRICE
                       from V_CA3301 CA,CFMAST CF,(
                                                        --BANG TAM SB
                                                        SELECT SB.CODEID, SB.SECTYPE, SB.SYMBOL,SB.ISINCODE,
                                                                CASE
                                                                     WHEN SB.SECTYPE IN ('003','006') THEN SB.BONDNAME
                       ...
                       WHERE CA.CUSTODYCD=CF.CUSTODYCD
                       AND CA.CODEID=SB.CODEID
                       AND SI.CODEID (+) = (CASE WHEN SB.REFCODEID IS NULL THEN SB.CODEID ELSE SB.REFCODEID END)
                       AND CA.CASTATUS='S' -- Yeu cau lay status SKQ sau 3340 (xac nhan)
                       AND CA.QTTY <> 0)
                       GROUP BY CUSTID,SYMBOL,ISINCODE,ISSUERID,SECURITIES_TYPE,SECURITIES_STATUS,
...
EXCEPTION
  WHEN OTHERS
   THEN
      plog.error ('od6032B: ' || SQLERRM || dbms_utility.format_error_backtrace);
      Return;
