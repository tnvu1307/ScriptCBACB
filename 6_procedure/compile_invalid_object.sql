SET DEFINE OFF;
CREATE OR REPLACE PROCEDURE compile_invalid_object(p_acctno varchar2 default null)
   IS
--
...
BEGIN
    ...
    (
        ...
        WHERE status = 'INVALID'
            AND object_type in ('FUNCTION','PROCEDURE','VIEW','TRIGGER','PACKAGE BODY','PACKAGE')
    )
    LOOP
        ...
        WHEN others THEN -- caution handles all exceptions
            PR_ERROR('COMPILE_ALL ERR','Compile: '||REC.OBJECT_NAME||' Error: ' || SQLERRM);
        END;
    END LOOP;
EXCEPTION
    WHEN OTHERS THEN
