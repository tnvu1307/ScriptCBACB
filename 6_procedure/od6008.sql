SET DEFINE OFF;
CREATE OR REPLACE PROCEDURE od6008(
    ...
    (
        ...
        FROM (
             SELECT
                ...
                GREATEST(SUM(CASE WHEN MST.SYMBOL LIKE '%_WFT' THEN 0 ELSE MST.TRADE + MST.NAMT_TRADE + MST.TRADE1404 END)) TRADE, --Them TRADE1404
            ...
            FROM (
                ...
                NVL(TR.NAMT_WITHDRAW,0) NAMT_WITHDRAW, MAX(SE.TRADE1404) TRADE1404
                FROM (
...
) GROUP BY FUNDNAME, ISSFULLNAME, CIFID, CUSTID, SYMBOL, ISINCODE, SECTYPE, INDATE, CDCONTENT
UNION ALL -------------------------------
