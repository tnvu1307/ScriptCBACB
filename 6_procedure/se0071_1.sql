...
-- GET REPORT'S DATA
 OPEN PV_REFCURSOR
 FOR
SELECT v_BRNAME TVLK,FULLNAME, CUSTODYCD, ACCTNO, IDCODE, IDDATE,IDPLACE, ADDRESS,MOBILE,MAX(RECUSTODYCD) RECUSTODYCD,MAX(RECUSTNAME)RECUSTNAME,
       DEPOSITNAME,
       v_sym311 sym311,v_sym312 sym312,v_sym313 sym313,v_sym314 sym314,v_sym315 sym315,v_sym316 sym316,v_sym317 sym317,v_sym318 sym318
FROM(SELECT CF.FULLNAME , CF.CUSTODYCD, CF.MCUSTID ACCTNO , SB2.SYMBOL SYMBOL,
       ...
       CASE WHEN SB.REFCODEID IS NULL THEN  'TD' ELSE 'WAIT'END LOAI_CK,
        CASE  when sb2.tradeplace='002' then 'HNX'
          ...
          else 'A' END SAN, MEM.FULLNAME DEPOSITNAME,--, PLSENT SENTO
            to_number(ou.trtype) LD_CK
FROM SESENDOUT OU,  VW_CFMAST_M CF, AFMAST AF, sbsecurities SB, sbsecurities SB2, deposit_member MEM,
...
GROUP BY FULLNAME, CUSTODYCD, ACCTNO, IDCODE, IDDATE,IDPLACE, ADDRESS,MOBILE,
       DEPOSITNAME
 ;
