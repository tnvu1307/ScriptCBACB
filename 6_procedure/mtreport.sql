...
BEGIN
    ...
    if PV_REPORTID in ('DE083') AND INSTR(PV_REPORTPARAM,'BRID:ALL') > 0 then
        ...
        LOOP
            ...
            IF L_COUNT >0 THEN
                FOR REC IN (
                    SELECT TRFCODE,BANKCODE FROM VSDTRFCODE WHERE TLTXCD = '9999' AND STATUS='Y' AND TYPE='REQ'
                )
                LOOP
                   Insert into VSD_PROCESS_LOG(AUTOID,TRFCODE,TLTXCD,TXNUM,TXDATE,PROCESS,TLID,BRID,BANKCODE)
                   values (SEQ_VSD_PROCESS_LOG.NEXTVAL,REC.TRFCODE, '9999',l_txnum,GETCURRDATE,'N',PV_TLID,PV_BRID,REC.BANKCODE);
                END LOOP;
            END IF;
        END LOOP;
    else
