SET DEFINE OFF;
CREATE OR REPLACE PROCEDURE cf6040 (
...
IS
    ...
    v_strReturnDate  varchar2(20);
    v_strBackDate  varchar2(20);
BEGIN

   V_STROPTION      := OPT;
   v_CurrDate       := getcurrdate;
   v_NextDate       := getnextbusinessdate(v_CurrDate, 1);
   v_BackDate       := getprevdate(v_CurrDate, 20);
   v_strCurrDate    := to_char (v_CurrDate, 'DD Mon YYYY');
   v_strReturnDate  := to_char (v_NextDate, 'DD Mon YYYY');
   v_strBackDate    := to_char (v_BackDate, 'DD Mon YYYY');
...
OPEN PV_REFCURSOR FOR
    ...
    select cf.custodycd, --so tk luu ky
        ...
        v_strReturnDate RETURNDATE,
        v_strBackDate BACKDATE,
...
EXCEPTION
  WHEN OTHERS
   THEN
      plog.error ('CF6040: ' || SQLERRM || dbms_utility.format_error_backtrace);
      Return;
End;
/
