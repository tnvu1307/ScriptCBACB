SET DEFINE OFF;
CREATE OR REPLACE PROCEDURE se00443 (
...
-- GET REPORT'S DATA
 ...
 FOR
        ...
        ,SUM(CAS.AMT + cas.cutamt + cas.sendamt) AMT,
         0 CP_LE, 0 RIGHT_QTTY, 0 CK_MUA, ' ' NOTE, CA.CIROUNDTYPE CIROUNDTYPE,
        ...
        WHERE CF.CUSTODYCD = V_CUSTODYCD AND CF.CUSTID = AF.CUSTID
                ...
                AND CAS.STATUS ='O'
                AND CAS.DELTD <> 'Y'
                GROUP BY CA.CAMASTID,CA.REPORTDATE,SB.TRADEPLACE,SB.MARKETTYPE,SB.SECTYPE,CA.CIROUNDTYPE
       UNION ALL
       ...
       0  REPORT_QTTY,
        0 CUTPBALANCE, 0  QTTY,
         0 CUTAQTTY
        , 0   AMT, 0 CP_LE, 0 RIGHT_QTTY, 0 CK_MUA, ' ' NOTE, 0 CIROUNDTYPE,' ' SAN_GD
       FROM DUAL
         ;
...
EXCEPTION
   WHEN OTHERS
   THEN
   plog.error('SE00443: Error: ' || SQLERRM || dbms_utility.format_error_backtrace);
      RETURN;
END;
