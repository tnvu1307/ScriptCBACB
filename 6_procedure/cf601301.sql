SET DEFINE OFF;
CREATE OR REPLACE PROCEDURE cf601301 (
...
OPEN PV_REFCURSOR FOR
    ...
    SELECT MST.CUSTODYCD, -- TK GIAO DICH CK
           ...
           -- KL SAU KHI GD
           MST.ENDQTTY,
           MST.ENDQTTY - MST.BEGINQTTY SL_GD,
           MST.BUY_QTTY,
           MST.SELL_QTTY,
           -- TY LE SO HU TRUOC KHI GD
           (
            CASE
                 WHEN MST.LISTINGQTTY = 0 AND MST.BEGINQTTY > 0  THEN 100
    ...
    (
        ...
        (
            ...
            NVL(SUM(CASE
                    WHEN P_SHARESOUTTYP='O' AND HISTDATE=V_FRPREVDATE THEN OLDCIRCULATINGQTTY
...
EXCEPTION
  WHEN OTHERS
   THEN
      PLOG.ERROR ('CF601301: ' || SQLERRM || DBMS_UTILITY.FORMAT_ERROR_BACKTRACE);
      RETURN;
END;
/
