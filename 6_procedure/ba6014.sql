SET DEFINE OFF;
CREATE OR REPLACE PROCEDURE ba6014 (
    ...
    SELECT * FROM(
        ...
        --'22/MAY/2017' CREATEDATE,
           ...
           -- (sb.valueofissue / (((sbr.pclosed * sbr.volume) / sbr.volume) * ab.qtty)) ltv, --ty le LTV
            ...
            btp.xparam,
            UTILS.FNC_NUMBER2WORK(btp.xparam) xparamw
        from sbsecurities sb, sbrefmrkdata sbr, allcode a1, sbsecurities ba,
        (select * from bondtype bot,bondissue boi where bot.issuesid =boi.issuesid) btp,
            (
                ...
                from (
                    select se.acctno, se.afacctno, se.bondcode,
                     ...
                     where se.status IN ('C')
                           ...
                           --and se.crplaceid is null
                     group by se.acctno, se.afacctno,se.bondcode
                )mt, (select acctno ,afacctno, bondcode codeid from bondsemast union all  select acctno,afacctno , codeid from semast where mortage > 0) se, sbsecurities sb, afmast af, cfmast cf, ISSUER_MEMBER iss, sbsecurities ba
        ...
        where
            ...
            and ab.bondsymbol = btp.bondsymbol
            and btp.xparam is not null
            and btp.xparam <> '0'
            AND sbr.txdate BETWEEN TO_DATE(getprevdate(I_DATE,NVL(btp.xparam,0)), 'DD/MM/YYYY') AND TO_DATE(I_DATE, 'DD/MM/YYYY')
       ...
       group by CREATEDATE, symbol,sectype,cdcontent,en_cdcontent,tradingdt,pclosed,volume,valueoftrading,
        qtty,secvalue,valueofissue,maxltvrate,xparam, xparamw
      ;

EXCEPTION
  WHEN OTHERS
   THEN
   DBMS_OUTPUT.PUT_LINE('BA6014 ERROR');
   PLOG.ERROR('BA6014: - ' ||DBMS_UTILITY.FORMAT_ERROR_BACKTRACE);
      RETURN;
END;
/
