SET DEFINE OFF;
CREATE OR REPLACE PROCEDURE dd6008(
...
Select   distinct
         ...
         CASE WHEN V_AMC ='%' THEN 'ALL' ELSE UPPER(FA.FULLNAME) END GROUP_NAME,
          getcurrdate from famembers fa where fa.roles ='AMC' and fa.shortname like V_AMC;

EXCEPTION
  WHEN OTHERS
   THEN
      plog.error ('DD6008: ' || SQLERRM || dbms_utility.format_error_backtrace);
      Return;
End;
/
