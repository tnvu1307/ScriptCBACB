SET DEFINE OFF;
CREATE OR REPLACE PROCEDURE ba6031(
...
OPEN PV_REFCURSOR FOR
    SELECT distinct BONDSYMBOL,to_char(CREATEDATE,'DD/MM/RRRR')CREATEDATE,CONTRACT,FULLNAMEBOND,GPDKKD,ADDRESS,PERIODINTEREST,TERM,
    ...
    SELECT  BSE.BONDSYMBOL,
            ...
            A2.cdcontent PERIODINTEREST,
            SB.ISSUEDATE,                 -- NGAY PHAT HANH
            SB.EXPDATE,                   -- NGAY DAO HAN
            sb.TERM,
            TO_CHAR(UTILS.SO_THANH_CHU(NVL(BSE.VALUEOFISSUE,0))) AS VALUEOFISSUE, --GIA TRI PHAT HANH
            TO_CHAR(UTILS.SO_THANH_CHU(NVL(SB.PARVALUE,0))) AS PARVALUE,                  -- MENH GIA
...
EXCEPTION
  WHEN OTHERS
   THEN
      PLOG.ERROR ('BA6031: ' || SQLERRM || DBMS_UTILITY.FORMAT_ERROR_BACKTRACE);
      RETURN;
END;
/
