SET DEFINE OFF;
CREATE OR REPLACE PROCEDURE od600604 (
...
,MST.TRANTYPE,AMOUNT,CLEARDATE FROM
(
SELECT MST.SECTYPE,MST.TRANTYPE,
    ...
    END
    ) AMOUNT,MST.SBDATE CLEARDATE,MST.CUSTODYCD
     FROM
(
SELECT CF.CUSTODYCD, SB.SBDATE, A1.*
    FROM (select * from cfmast where custodycd not like 'SHVE%' AND COUNTRY='234') CF,
  (
...
(
    ...
    WHERE OD.DELTD <> 'Y' AND CLEARDATE BETWEEN V_FROMDATE AND V_TODATE
...
GROUP BY MST.SECTYPE,MST.TRANTYPE,MST.AMOUNT,MST.CLEARDATE) RM ;

EXCEPTION
  WHEN OTHERS
   THEN
      PLOG.ERROR ('OD600604: ' || SQLERRM || DBMS_UTILITY.FORMAT_ERROR_BACKTRACE);
      RETURN;
END;
/
