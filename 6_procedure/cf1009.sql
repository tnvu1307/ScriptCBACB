...
-- return;
    ...
    OPEN PV_REFCURSOR FOR
        ...
        (
            ...
            select max(tse.txnum) txnum, max(tse.autoid) orderid, tse.custid, tse.afacctno, max(tse.tllog_autoid) autoid,tse.txtype,tse.txcd,
                tse.busdate, case when tse.TLTXCD = '8867' then 'B?n ' || trim(to_char(sum(tse.namt),'999,999,999,999,999,999')) || ' ' || max(tse.symbol) || ' ng?y ' || TO_CHAR(tse.busdate)
        ...
        0 ci_credit_amt, 0 ci_debit_amt,
            ...
            select tci.txnum, tci.autoid orderid, tci.custid, tci.acctno afacctno,
            ...
            where tci.busdate >= v_FromDate AND tci.busdate <= v_ToDate
              ...
              select max(tci.txnum) txnum,  max(tci.autoid) orderid, tci.custid, tci.acctno afacctno,
              max(CASE WHEN tci.tltxcd IN ('2678','7702','5566','2695','2674','2679','1156','2624') THEN 0000000001
                 ...
                 tci.busdate,
                 case when TCI.TLTXCD = '8865' then 'Tra tien mua CK ngay ' || to_char(max(od.txdate),'dd/mm/rrrr')--TO_CHAR(tci.busdate)
                    when TCI.TLTXCD = '8889' then 'Tra tien mua CK ngay' || to_char(max(od.txdate),'dd/mm/rrrr')--TO_CHAR(tci.busdate)
                    when TCI.TLTXCD = '8855' then 'Tra phi mua CK ngay' || to_char(max(od.txdate),'dd/mm/rrrr')--TO_CHAR(tci.busdate)
                    when TCI.TLTXCD = '8866' then 'Nhan tien ban CK ngay' || to_char(max(od.txdate),'dd/mm/rrrr')--TO_CHAR(tci.busdate)
                    else  'Tra phi ban CK ngay' || to_char(max(od.txdate),'dd/mm/rrrr')--TO_CHAR(tci.busdate)
                    end TXDESC,
                '' symbol, 0 se_credit_amt, 0 se_debit_amt,
