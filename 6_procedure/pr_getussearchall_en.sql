...
is
    ...
    */
begin
    --PLOG.ERROR('TellerId: '||TellerId || ' BranchId: '||BranchId||' SearchFilter:'||SearchFilter);
    --PLOG.ERROR('tlG: '||TellerGroup);
    if TellerId='0001' THEN --Admin he thong
        v_sql:='SELECT distinct *
        FROM (
        SELECT lg0.CFFULLNAME CFFULLNAME,lg0.CFCUSTODYCD IDAFACCTNO, cf.cifid, NVL(SB.SYMBOL,''---'') CODEID, tltx.en_txdesc NAMENV,
        ...
        FROM (select * from tllogall where batchname = ''DAY'' or tltxcd in (''0381'',''3375'')) lg0, ALLCODE A0, ALLCODE A1,tltx,SBSECURITIES SB, cfmast cf,
          (
        ...
        WHERE NOT (lg0.TLTXCD LIKE ''%71'' OR lg0.TLTXCD LIKE ''%72'' OR lg0.TLTXCD LIKE ''%6631''  OR lg0.TLTXCD IN (''0011'',''1175'',''8873'',''2282'',''1177''))
          ...
          AND lg0.ccyusage= SB.CODEID(+) and lg0.CFCUSTODYCD = cf.custodycd (+)
        ...
        SELECT distinct * FROM (SELECT lg0.CFFULLNAME CFFULLNAME, lg0.CFCUSTODYCD IDAFACCTNO, cf.cifid, NVL(SB.SYMBOL,''---'') CODEID,
            ...
            FROM (select * from tllogall where batchname = ''DAY'' or tltxcd in (''0381'',''3375'')) lg0, cfmast cf,
