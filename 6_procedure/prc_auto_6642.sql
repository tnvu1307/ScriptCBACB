SET DEFINE OFF;
CREATE OR REPLACE PROCEDURE prc_auto_6642(P_AUTOID NUMBER, P_REFVAL VARCHAR2, P_ERR_CODE OUT VARCHAR2)
IS
    PKGCTX   PLOG.LOG_CTX;
    ...
    L_AITHERAMOUNT NUMBER;
BEGIN
    PLOG.SETBEGINSECTION(PKGCTX, 'PRC_AUTO_6642');
    ...
    FOR REC IN
    (
        SELECT DD.AFACCTNO, DD.ACCTNO, NVL(FEE.FEEAMOUNT, 0) + NVL(FEE.TAXAMOUNT, 0) AMOUNT, DD.BALANCE, FEE.CURRENCY FEECCYCD, DD.CCYCD DDCCYCD, FEE.REMARK REMARK_DESC,
            CF.CUSTODYCD, CF.CIFID, CF.FULLNAME, CF.EMAIL, DD.REFCASAACCT
        FROM CFMAST CF,
        (
    ...
    LOOP
        ...
        IF REC.FEECCYCD <> REC.DDCCYCD THEN
            BEGIN
                SELECT NVL(AITHERTXAMT, 0), NVL(EXCHANGERATE, 0) INTO L_AMOUNT, L_EXCHANGERATE
                FROM SETTLE_FEE_LOG
                WHERE FEEBOOKINGAUOID = P_AUTOID AND STATUS = 'C';

                L_AITHERAMOUNT := L_AMOUNT;
        ...
        --CATXNUM
         ...
         IF TXPKS_#6642.FN_AUTOTXPROCESS(L_TXMSG, P_ERR_CODE, L_ERR_PARAM) <> SYSTEMNUMS.C_SUCCESS THEN
             PLOG.ERROR(PKGCTX, ' RUN ' || '6642' || ' GOT ' || P_ERR_CODE || ':' || P_ERR_CODE);
             L_ERR_PARAM := 'ERR: ' || SQLERRM || ' TRACE: ' || DBMS_UTILITY.FORMAT_ERROR_BACKTRACE;
             ROLLBACK;
             PLOG.SETENDSECTION(PKGCTX, 'PRC_AUTO_6642');
             RETURN;
         END IF;
         /*
         l_data_source :=  'select ''' || REC.FULLNAME || ''' p_fullname,
         ''' || REC.CIFID || ''' p_cifid,
         ''' || REC.CUSTODYCD || ''' p_custodycd,
         ''' || REC.FEECCYCD || TO_CHAR(REC.AMOUNT, 'FM999,999,999,999,990.009999') || ''' p_feeamt,
         ''' || REC.DDCCYCD || TO_CHAR(L_AITHERAMOUNT, 'FM999,999,999,999,990.009999') || ''' p_aitheramt,
         ''' || L_EXCHANGERATE || ''' p_exchangerate,
         ''' || REC.REFCASAACCT || ''' p_refcasaacct,
         ''' || L_STRDESC || ''' p_desc,
         ''' || TO_CHAR(L_CURRDATE, 'DD/MM/RRRR') || ''' p_txdate from dual';
         NMPKS_EMS.INSERTEMAILLOG(REC.EMAIL, 'EM47', l_data_source, NULL);
         */
    END LOOP;
    PLOG.SETENDSECTION(PKGCTX, 'PRC_AUTO_6642');
EXCEPTION
