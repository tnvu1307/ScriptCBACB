SET DEFINE OFF;
CREATE OR REPLACE PROCEDURE redht003 (
...
select TEN_NHOM, round(sum(EXECAMT),0) EXECAMT, round(sum(FEEACR),0) FEEACR, round(sum(MRAMT),0) MRAMT, V_NDATE NDATE, F_DATE indate
from
(
...
(
    ...
    WHERE TN.AUTOID = NHOM.REFRECFLNKID AND NHOM.STATUS = 'A'
        AND TN.AUTOID <> '17'
        and tn.grptype = 'R' and TN.autoid = V_GROUPID
) REG
ON MAIN.SO_TK_MG = REG.REACCTNO
GROUP BY  MAIN.TXDATE, REG.TEN_NHOM, REG.REACCTNO
)
