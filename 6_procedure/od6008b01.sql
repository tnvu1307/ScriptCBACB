SET DEFINE OFF;
CREATE OR REPLACE PROCEDURE od6008b01(
...
FROM (
     SELECT
        ...
        MST.CCYCD,MST.MCUSTODYCD,(case when V_MCUSTODYCD = '%' then '' else MST.MFULLNAME end) MFULLNAME,(case when V_MCUSTODYCD = '%' then '' else MST.MCIFID2 end) MCIFID2
    FROM (
        ...
        SELECT SE.FUNDNAME, SE.CIFID, SE.CUSTID, SE.SYMBOL, SE.ISINCODE, SE.FULLNAME, SE.BONDNAME, SE.ACCTNO, SE.NETTING, SE.RECEIVING, SE.TRADE, NVL(TR.NAMT_NETTING,0) NAMT_NETTING,
               ...
               NVL(TR.NAMT_MORTAGE,0) NAMT_MORTAGE, SE.CCYCD, SE.PARVALUE, SE.TRADE1404, SE.MCUSTODYCD, SE.MFULLNAME, SE.MCIFID2
        FROM (
            SELECT CF.FULLNAME FUNDNAME, CF.CIFID, CF.CUSTID, REPLACE(SB.SYMBOL,'_WFT','') SYMBOL, SB.ISINCODE, ISS.FULLNAME, SB.BONDNAME,
                   SE.ACCTNO ,SE.NETTING, SE.RECEIVING, SE.BLOCKED, SE.EMKQTTY, SE.TRADE, SE.MORTAGE, SC.SHORTCD CCYCD, SB.PARVALUE, SE.TRADE1404,
                   CF.MCUSTODYCD, CF.MFULLNAME, CF.MCIFID2
            ...
            GROUP BY CF.FULLNAME, CF.CIFID, CF.CUSTID, ISS.FULLNAME, SB.BONDNAME, SE.ACCTNO ,SE.NETTING,
                     ...
                     REPLACE(SB.SYMBOL,'_WFT',''), SB.ISINCODE, CF.MCUSTODYCD, CF.MFULLNAME, CF.MCIFID2
        ) SE
        LEFT JOIN (
        ...
        ,SE.MCUSTODYCD, SE.MFULLNAME, SE.MCIFID2
    )MST
    GROUP BY MST.FUNDNAME, MST.CIFID, MST.CUSTID, MST.FULLNAME, MST.BONDNAME, MST.SYMBOL, MST.ISINCODE, MST.CCYCD, MST.PARVALUE, MST.MCUSTODYCD, MST.MFULLNAME, MST.MCIFID2
)RT
...
EXCEPTION
  WHEN OTHERS
   THEN
      PLOG.ERROR ('OD6008B01: ' || SQLERRM || DBMS_UTILITY.FORMAT_ERROR_BACKTRACE);
      RETURN;
END;
/
