SET DEFINE OFF;
CREATE OR REPLACE PROCEDURE bo1014 (
...
-- BO1014: Phieu lenh ban TP/TPKB(Sell on bond/T-bill order)
...
select od.txdate, od.txtime, od.orderid, cf.custodycd, cf.fullname, af.acctno afacctno, sb.symbol,
    ...
    where od.afacctno=af.acctno
        ...
        and not EXISTS (select * from tbl_odrepo tb where tb.orderid=od.orderid or tb.ref_orderid=od.orderid)
...
EXCEPTION
   WHEN OTHERS
   THEN
    plog.error('BO1014.' || sqlerrm || '.At:' || dbms_utility.format_error_backtrace);
      RETURN;
END;
/
