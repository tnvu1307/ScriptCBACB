...
OPEN PV_REFCURSOR  FOR
  SELECT
      ROW_NUMBER() OVER (ORDER BY SEB.SYMBOL) NO, V_DATE REPORTDATE,
      SEB.SYMBOL,
      sum(SE.TRADE - NVL(TR.TRADE,0)+ SE.RECEIVING- NVL(TR.RECEIVING,0)- SE.NETTING + NVL(TR.NETTING,0))  QTTY_T,
      CIRCULATINGQTTY_T,
      CAST(sum(SE.TRADE - NVL(TR.TRADE,0)+ SE.RECEIVING- NVL(TR.RECEIVING,0)- SE.NETTING + NVL(TR.NETTING,0))/ CIRCULATINGQTTY_T as numeric(38,10)) RATE_T,
      CAST(sum(SE.TRADE - NVL(PRETR.TRADE,0)+ SE.RECEIVING- NVL(PRETR.RECEIVING,0)- SE.NETTING + NVL(PRETR.NETTING,0))/ CIRCULATINGQTTY_PRET as numeric(38,10)) RATE_PRET,
      CAST(sum(SE.TRADE - NVL(TR.TRADE,0)+ SE.RECEIVING- NVL(TR.RECEIVING,0)- SE.NETTING + NVL(TR.NETTING,0))/ CIRCULATINGQTTY_T as numeric(38,10)) - CAST(sum(SE.TRADE - NVL(PRETR.TRADE,0)+ SE.RECEIVING- NVL(PRETR.RECEIVING,0)- SE.NETTING + NVL(PRETR.NETTING,0))/ CIRCULATINGQTTY_PRET as numeric(38,10)) DELTA_CHANGE
FROM CFMAST CF, AFMAST AF,SBSECURITIES SEB, SEMAST SE ,
...
GROUP BY SEB.SYMBOL,CIRCULATINGQTTY_T;
EXCEPTION
   WHEN OTHERS
   THEN
