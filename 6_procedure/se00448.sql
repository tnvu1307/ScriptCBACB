SET DEFINE OFF;
CREATE OR REPLACE PROCEDURE se00448 (
...
-- GET REPORT'S DATA
 ...
 FOR
        ...
        '005',CA.DEVIDENTSHARES,'010', CA.DEVIDENTRATE,'023',CA.EXRATE,'015', ca.interestrate || '(%)', '016',ca.interestrate|| '(%)', '1/1')) CA_TYLE,
        MAX(CAS.TRADE)  REPORT_QTTY, SUM(CAS.CUTPBALANCE) CUTPBALANCE,
     MAX(CAS.QTTY + cas.cutqtty + cas.sendqtty)  QTTY,
        ...
        WHERE CF.CUSTODYCD = V_CUSTODYCD AND CF.CUSTID = AF.CUSTID
                ...
                AND CA.CATYPE NOT IN ('011','010','014','020','017','021') AND AF.ACCTNO LIKE V_STRAFACCTNO
...
EXCEPTION
   WHEN OTHERS
   THEN
   plog.error('SE00448: Error: ' || SQLERRM || dbms_utility.format_error_backtrace);
      RETURN;
END;
/
