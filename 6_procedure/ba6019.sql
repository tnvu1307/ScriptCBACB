SET DEFINE OFF;
CREATE OR REPLACE PROCEDURE ba6019(
...
OPEN PV_REFCURSOR FOR
            WITH TMP AS (
                    SELECT DISTINCT V_IDATE CREATEDATE,
                                    (CASE WHEN V_TXNUM IS NULL THEN '...../' ELSE V_TXNUM||'/' END)||TO_CHAR(V_currdate,'yyyy')||'/SSD-SHBVN' REPORTINGNO,
                                    ISS.FULLNAME FULLNAMEBOND, --TEN TCPH
                                    MST.CONTRACTNO, --SO HOP DONG
                                    MST.CONTRACTDATE, --NGAY HOP DONG
                                    RS.SYMBOL, --MA TRAI PHIEU
                                    TO_CHAR(RS.ISSUEDATE,'DD/MM/RRRR') ISSUEDATE, -- NGAY PHAT HANH
                                    RS.EXPDATE, -- NGAY DAO HAN
                                    TO_CHAR(UTILS.SO_THANH_CHU(NVL(BO.ISSUEVAL,0))) AS VALUEOFISSUE, --GIA TRI PHAT HANH
                                    RS.PARVALUE, -- MENH GIA
                                    RS.CCYCD, -- DV TIEN TE
                                    RS.INTCOUPON, -- LAI SUAT
                                    CF.FULLNAME --TRAI CHU
                                    ...
                                    --RS.TRADE,
                                    --RS.BONDSYMBOL--MA TRAI PHIEU NAM GIU
                            FROM
                                (
                                 SELECT BSE.AFACCTNO, BSE.BONDSYMBOL, SB.ISSUERID, SB.ISSUEDATE, SB.EXPDATE, SB.VALUEOFISSUE
                                      , SB.PARVALUE, SB.INTCOUPON, SB.CONTRACTNO, SB.CONTRACTDATE, SB.CCYCD
                                 ...
                                 FROM (
                                        SELECT SB.CODEID, SB.SECTYPE, SB.SYMBOL, SB1.CODEID REFCODEID, SB1.SYMBOL REFSYMBOL
                                              ...
                                              , SB.INTCOUPON, SB.CONTRACTNO, SB.CONTRACTDATE, CCY.SHORTCD AS CCYCD
                                        FROM SBSECURITIES SB, SBSECURITIES SB1,SBCURRENCY CCY
                                        WHERE SB.REFCODEID = SB1.CODEID(+) AND
                                              SB.SECTYPE IN ('003','006') AND
                                              SB.SECTYPE <> '004' AND
                                              SB.CCYCD = CCY.CCYCD AND
                                 ...
                                 ISSUERS ISS, CFMAST CF, AFMAST AF, ISSUES IE, BONDISSUE BO,(SELECT ISS.ISSUECODE,ISS.ISSUERID,BI.BONDCODE,MST.CONTRACTNO,MST.CONTRACTDATE FROM ISSUER_CONTRACTNO MST, ISSUES ISS,BONDISSUE BI
                            ...
                            WHERE RS.ISSUERID = ISS.ISSUERID AND
                                ...
                                AND BO.BONDCODE=MST.BONDCODE (+)

                        )
            SELECT TMP1.*,TMP2.*
            FROM TMP TMP1, (
                           SELECT LISTAGG(SYMBOL,', ') WITHIN GROUP (ORDER BY CREATEDATE) LIST_SYMBOL
                           FROM (
                                 SELECT DISTINCT CREATEDATE, SYMBOL
                                 FROM TMP
                                )
                           ) TMP2;
EXCEPTION
  WHEN OTHERS
   THEN
      PLOG.ERROR ('BA6019: ' || SQLERRM || DBMS_UTILITY.FORMAT_ERROR_BACKTRACE);
      RETURN;
END;
/
