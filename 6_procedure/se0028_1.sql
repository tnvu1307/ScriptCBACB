SET DEFINE OFF;
CREATE OR REPLACE PROCEDURE se0028_1 (
...
SELECT
         ...
         nvl(cf.address ,'')address,
         nvl(cf.idplace ,'')idplace,
         ...
         REPLACE(nvl(sb.symbol,''),'_WFT','')  PV_SYMBOL
  FROM   (
 ...
 WHERE       tl.acctno = af.acctno
         ...
         AND sb.tradeplace IN ('001', '002', '005','006','010', '009')
...
----         AND sb.tradeplace = A2.cdval
        ...
        Group BY cf.fullname,cf.custodycd,cf.idcode ,cf.iddate ,A1.cdval,sb.symbol,iss.fullname,sb.PARVALUE,A2.cdcontent,
            tl.TYPE, tl.securitiestype,cf.tradingcode,cf.tradingcodedt,cf.address,cf.idplace,cf.IDTYPE
) T
 ;
