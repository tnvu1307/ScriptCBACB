SET DEFINE OFF;
CREATE OR REPLACE PROCEDURE se00362 (
...
SELECT * FROM (
        ...
        FROM (
            SELECT
                ...
                NVL(ROUND(SUM(CAS.AMT + cas.cutamt + cas.sendamt )/DECODE (MAX(CA.EXPRICE),0,1,MAX(CA.EXPRICE)),3),MAX(CA.CIROUNDTYPE))  CP_LE,
...
EXCEPTION
  WHEN OTHERS
   THEN
   DBMS_OUTPUT.PUT_LINE('SE00362 ERROR');
   PLOG.ERROR('SE00362: - ' ||DBMS_UTILITY.FORMAT_ERROR_BACKTRACE);
      RETURN;
END;
/
