SET DEFINE OFF;
CREATE OR REPLACE PROCEDURE od600801_amc(
...
FROM (
    ...
    FROM (
        ...
        FROM (
            ...
            FROM SEMAST SE,SBSECURITIES SB, ISSUERS ISS, SBCURRENCY SC, AFMAST AF,
            (
                SELECT CF.*
                FROM CFMAST CF
            ...
            GROUP BY CF.FULLNAME, CF.CIFID, CF.CUSTID, ISS.FULLNAME, SB.BONDNAME, SE.ACCTNO ,SE.NETTING,
                     ...
                     REPLACE(SB.SYMBOL,'_WFT',''), SB.ISINCODE
        ) SE
        LEFT JOIN (
...
EXCEPTION
  WHEN OTHERS
   THEN
      PLOG.ERROR ('od600801_GCB: ' || SQLERRM || DBMS_UTILITY.FORMAT_ERROR_BACKTRACE);
      RETURN;
END;
/
