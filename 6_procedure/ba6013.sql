SET DEFINE OFF;
CREATE OR REPLACE PROCEDURE ba6013 (
...
BEGIN
    ...
    FOR
        SELECT ISS.AUTOID ISSUESID,
               SB.CODEID,
               SB.SYMBOL,
               V_REPORTDATE VALUATION_DATE,
               ISS.VALUEOFISSUE,
               (CASE WHEN SB.SECTYPE IN ('001','008','011','003','006') THEN 'SECURITIES'
                    ELSE UPPER(A1.EN_CDCONTENT)
               END) TYPE,
               ISS.MAXLTVRATE/100 MAXLTVRATE,
               A2.CDCONTENT TYPERATE_NAME,
               SB.PARVALUE,
               A1.CDCONTENT SECTYPE_NAME,
               A1.EN_CDCONTENT EN_SECTYPE_NAME,
               SUM(MT.QTTY) QTTY,
               ROUND(
                    (CASE WHEN ISS.TYPERATE = V_TYPERATE THEN ISS.LTVRATE
                          ELSE (CASE WHEN ISS.LTVRATE = 0 THEN 0 ELSE 1/(ISS.LTVRATE/100)*100 END)
                     END
                    )/100
               ,10) LTVRATE,
                ...
                SELECT MK.SYMBOL, BO.ISSUESID,
                       ROUND(
                            CASE WHEN BO.SECTYPE = '111' THEN (
                                 ...
                                 ELSE (CASE WHEN SUM(MK.TRANVALUE) = 0 THEN 0 ELSE SUM(MK.TRANVALUE)/SUM(MK.VOLUME) END) END)
                            ELSE 0 END
                       ,10) SHAREPRICE
