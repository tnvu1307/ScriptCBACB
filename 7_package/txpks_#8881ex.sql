SET DEFINE OFF;
CREATE OR REPLACE PACKAGE txpks_#8881ex
/**----------------------------------------------------------------------------------------------------
 ** Package: TXPKS_#8881EX
...
CREATE OR REPLACE PACKAGE BODY txpks_#8881ex
IS
   pkgctx   plog.log_ctx;
    ...
    IF p_txmsg.deltd <> 'Y' THEN -- Normal transaction
         if p_txmsg.txfields ('31').value = 'SE' then
            ...
            UPDATE ODMAST
                ...
                WHERE orderid=p_txmsg.txfields('35').value;
            update stschd
                set status = 'C',
                    LASTCHANGE = SYSTIMESTAMP
                where orderid = p_txmsg.txfields('35').value and duetype = 'SS';
         else
            -- LAY TK TONG DE DI TIEN BAN
                ...
                BEGIN
                    ...
                    if P_ERR_CODE = systemnums.C_SUCCESS then
                        ...
                        VALUES (p_txmsg.txnum, TO_DATE (p_txmsg.txdate, systemnums.C_DATE_FORMAT),p_txmsg.txfields('04').value,'0009',ROUND(p_txmsg.txfields('09').value,0),NULL,'',p_txmsg.deltd,'',seq_DDTRAN.NEXTVAL,p_txmsg.tltxcd,p_txmsg.busdate,'' || '' || '');
