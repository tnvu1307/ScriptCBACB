...
IS
   ...
   c_bseacctno        CONSTANT CHAR(2) := '05';
...
BEGIN
    ...
    ***************************************************************************************************/
    ...
    ELSE -- Revert GD
        ...
        select count(*) into l_count
        from ESCROW es
        where upper(es.ESCROWID) = upper(trim(p_txmsg.txfields('01').value))  and es.sestatus in ('PC')
            and DELTD <> 'Y';
        if l_count = 0 then
            p_err_code := '-180022';
...
BEGIN
    ...
    ***************************************************************************************************/
    IF p_txmsg.deltd <> 'Y' THEN
        update ESCROW
        set payqtty = payqtty + TO_NUMBER(p_txmsg.txfields('10').value),
            pstatus = pstatus||'|'||status, status = decode(status,'PC','C','PS'),
            sestatus = 'CC',
    ...
    ELSE --Revert trans
        update ESCROW
        set payqtty = payqtty - TO_NUMBER(p_txmsg.txfields('10').value),
            pstatus = pstatus||'|'||status, status = decode(status,'C','PC','A'), sestatus = 'AC',
            last_change = CURRENT_TIMESTAMP
        where ESCROWID = p_txmsg.txfields('01').value and deltd = 'N';
    END IF;
