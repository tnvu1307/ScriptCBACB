...
IS
   ...
   c_typeofdocument   CONSTANT CHAR(2) := '86';
   c_status           CONSTANT CHAR(2) := '81';
FUNCTION fn_txPreAppCheck(p_txmsg in tx.msg_rectype,p_err_code out varchar2)
RETURN NUMBER
IS
Status varchar(2);
BEGIN
    ...
    * END IF;
    ***************************************************************************************************/
    select status into Status from CFVSDLOG where txnum = p_txmsg.txfields('87').value;
    IF Status LIKE 'R' THEN
        p_err_code := '-901217';
        plog.setendsection (pkgctx, 'fn_txPreAppCheck');
        RETURN errnums.C_BIZ_RULE_INVALID;
    END IF;
