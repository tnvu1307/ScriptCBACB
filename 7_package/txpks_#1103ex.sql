SET DEFINE OFF;
CREATE OR REPLACE PACKAGE txpks_#1103ex
/**----------------------------------------------------------------------------------------------------
 ** Package: TXPKS_#1103EX
...
CREATE OR REPLACE PACKAGE BODY txpks_#1103ex
IS
   pkgctx   plog.log_ctx;
...
BEGIN
    ...
    ***************************************************************************************************/
    ...
    IF p_txmsg.deltd <> 'Y' THEN
        ...
        select count(*) into l_count
        from ESCROW es
        where upper(es.ESCROWID) = upper(trim(p_txmsg.txfields('01').value)) and sestatus in ('BC')
            and DELTD <> 'Y';
        if l_count = 0 then
            p_err_code := '-180022';
        ...
        BEGIN
            SELECT SUM ( QTTY) QTTY INTO l_qtty_standing FROM SEMORTAGE WHERE STATUS ='N' AND DELTD <>'Y' AND acctno =   p_txmsg.txfields('03').value;
              EXCEPTION
              WHEN OTHERS
              THEN
          l_qtty_standing:=0;
        END ;
         ...
         IF NOT (to_number(l_EMKQTTY)- nvl(l_qtty_standing,0) >= to_number(p_txmsg.txfields('10').value)) THEN
            p_err_code := '-900143';
    ...
    ELSE -- Revert GD
        ...
        select count(*) into l_count
        from ESCROW es
        where upper(es.ESCROWID) = upper(trim(p_txmsg.txfields('01').value)) and sestatus in ('AC')
            and DELTD <> 'Y';
        if l_count = 0 then
            p_err_code := '-180022';
    ...
    ***************************************************************************************************/
    ...
    IF p_txmsg.deltd <> 'Y' THEN
        ...
        set blockedqtty = blockedqtty - TO_NUMBER(p_txmsg.txfields('10').value),
            pstatus = pstatus||'|'||status, status = decode(status,'B','BC','A'),
            sestatus='AC',
