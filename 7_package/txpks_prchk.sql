...
BEGIN
    --30/11/2016, TruongLD modified: neu phat sinh trong ngay --> tinh them log phat sinh
    select varvalue into l_HOSTATUS from sysvar where varname='HOSTATUS';
    If l_HOSTATUS = systemnums.C_OPERATION_ACTIVE Then
        BEGIN
            SELECT nvl(sum(prinused),0) INTO l_ExpectUsed
            FROM prinusedlog
