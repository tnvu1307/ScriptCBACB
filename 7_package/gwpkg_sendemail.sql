...
begin
    ...
    plog.setbeginsection (pkgctx, 'fnc_sendOTPemail');
    ...
    If v_istest ='Y' then
        if INSTR(v_emailtest, lower(p_receivermail)) > 0 then
            ...
            EXECUTE IMMEDIATE v_sql  USING p_seq, p_receivername, p_receivermail, p_datasource, out v_return_code, out v_return_msg, out v_err_code;
        else
            p_return_msg := 'Dang moi truong test, chi gui mail trong DS:' || v_emailtest;
            Return;
        End if;
    Else
         insert into EMAILOG_OUT(seq , receivername ,  receivermail ,    subject ,  strBody) values(p_seq, p_receivername, p_receivermail, '', p_datasource);
         v_sql :='BEGIN MAILUSER_API.SP_NVREALTIMEACCEPT_API_GEN@REPAY_MAIL(:v_SEQ, :v_RECEIVER_NM, :v_RECEIVER, :v_JONMUN, :v_RTN_CD, :v_RTN_MSG, :v_ER_CD); END;';
        EXECUTE IMMEDIATE v_sql  USING p_seq, p_receivername, p_receivermail, p_datasource, out v_return_code, out v_return_msg, out v_err_code;
    ...
    plog.setendsection (pkgctx, 'fnc_sendOTPemail');
exception
when others
   then
