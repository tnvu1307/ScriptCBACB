...
/** ----------------------------------------------------------------------------------------------------
 ** Module: TX
 ** Description: Allocate cash dividend to sub account - tax at issuer
...
plog.setbeginsection (pkgctx, 'fn_txAppAutoCheck');
   IF p_txmsg.deltd = 'N' THEN
     ...
     If txpks_check.fn_aftxmapcheck(p_txmsg.txfields('21').value,'DDMAST','21','3354')<>'TRUE' then 
         p_err_code := errnums.C_SA_TLTX_NOT_ALLOW_BY_ACCTNO;
         plog.setendsection (pkgctx, 'fn_txAppAutoCheck');
         RETURN errnums.C_BIZ_RULE_INVALID;
     End if;

     l_DDMASTcheck_arr := txpks_check.fn_DDMASTcheck(p_txmsg.txfields('21').value,'DDMAST','ACCTNO');
   ...
   IF p_txmsg.deltd <> 'Y' THEN -- Normal transaction
    ...
    IF to_char(p_txmsg.txfields('61').value) <> '0' THEN 
      INSERT INTO SETRAN(TXNUM,TXDATE,ACCTNO,TXCD,NAMT,CAMT,ACCTREF,DELTD,REF,AUTOID,TLTXCD,BKDATE,TRDESC)
            VALUES (p_txmsg.txnum, TO_DATE (p_txmsg.txdate, systemnums.C_DATE_FORMAT),p_txmsg.txfields ('08').value,'0011',ROUND(p_txmsg.txfields('11').value,0),NULL,p_txmsg.txfields ('01').value,p_txmsg.deltd,p_txmsg.txfields ('01').value,seq_SETRAN.NEXTVAL,p_txmsg.tltxcd,p_txmsg.busdate,'' || '' || '');
    END IF;
