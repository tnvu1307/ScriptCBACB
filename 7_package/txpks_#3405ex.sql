SET DEFINE OFF;
CREATE OR REPLACE PACKAGE txpks_#3405ex
/**----------------------------------------------------------------------------------------------------
 ** Package: TXPKS_#3405EX
...
CREATE OR REPLACE PACKAGE BODY txpks_#3405ex
IS
   pkgctx   plog.log_ctx;
...
BEGIN
    ...
    ***************************************************************************************************/
    IF P_TXMSG.DELTD <> 'Y' THEN
        SELECT COUNT(1) INTO L_COUNT FROM CA040REGISTER WHERE AUTOID = TO_NUMBER(P_TXMSG.TXFIELDS('99').VALUE) AND MSGSTATUS IN ('S') AND DELTD = 'N';
        IF L_COUNT = 0 THEN
            p_err_code := '-100165';
            plog.setendsection (pkgctx, 'fn_txPreAppCheck');
            RETURN errnums.C_BIZ_RULE_INVALID;
...
BEGIN
    ...
    ***************************************************************************************************/
    IF P_TXMSG.DELTD <> 'Y' THEN
        ...
        UPDATE CA040REGISTER SET MSGSTATUS = 'R' WHERE AUTOID = L_AUTOID;
    END IF;
