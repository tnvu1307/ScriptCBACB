SET DEFINE OFF;
CREATE OR REPLACE PACKAGE txpks_#1218ex
/**----------------------------------------------------------------------------------------------------
 ** Package: TXPKS_#1218EX
...
CREATE OR REPLACE PACKAGE BODY txpks_#1218ex
IS
   pkgctx   plog.log_ctx;
...
BEGIN
    ...
    ***************************************************************************************************/
    IF P_TXMSG.DELTD <> 'Y' THEN
        ...
        FOR REC IN (
            SELECT FEE.AUTOID, FEE.FEECODE, CF.CUSTODYCD, NVL(CF.SETTLETYPE, '60') SETTLETYPE, FEE.BANKACCOUNT BANKACCTNO, FEE.SPRACNO,
                FEE.FEEAMOUNT, NVL(FEE.TAXAMOUNT,0) TAXAMOUNT, ROUND((NVL(FEE.FEEAMOUNT,0) + NVL(FEE.TAXAMOUNT,0)),2) FEEAMTVAT,
                FEE.CURRENCY CCYCD, 'Cancel the fee settlement (' || FEE.REMARK || ')' SETTREMARK
            ...
            IF TXPKS_#1208.FN_BATCHTXPROCESS(L_TXMSG, P_ERR_CODE, L_ERR_PARAM) <> SYSTEMNUMS.C_SUCCESS THEN
                plog.error (pkgctx, SQLERRM || dbms_utility.format_error_backtrace);
                CONTINUE;
            END IF;
        END LOOP;
