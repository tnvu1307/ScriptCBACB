SET DEFINE OFF;
CREATE OR REPLACE PACKAGE txpks_#6703ex
/**----------------------------------------------------------------------------------------------------
 ** Package: TXPKS_#6703EX
...
CREATE OR REPLACE PACKAGE BODY txpks_#6703ex
IS
   pkgctx   plog.log_ctx;
   ...
   c_sacctno          CONSTANT CHAR(2) := '06';
   c_sname            CONSTANT CHAR(2) := '07';
   c_racctno          CONSTANT CHAR(2) := '08';
   c_exrate           CONSTANT CHAR(2) := '09';
   c_examt            CONSTANT CHAR(2) := '10';
   c_note             CONSTANT CHAR(2) := '11';
...
BEGIN
    ...
    ***************************************************************************************************/
    IF p_txmsg.deltd <> 'Y' THEN
     ...
     v_refcasaacct:=p_txmsg.txfields('08').value;
     begin
        select cf.bondagent,cf.custodycd,dd.accounttype into l_bondagent,v_custodycd,v_to_accounttype
     ...
     exception when NO_DATA_FOUND
        ...
        if v_to_accounttype = 'IICA' and p_txmsg.txfields('05').value = 'USD' then  --Cash amount(After exchange)
     ...
     if  p_txmsg.txfields('05').value = 'USD' then
         ...
         if l_bondagent <> 'Y' and substr(v_custodycd,0,4) <> l_sysvar then
            v_Result := cspks_feecalc.FN_CB_OVERSEAS_CALC(v_custodycd, v_amt, 0,'016', v_feecd, v_feeamt, v_feerate, v_ccycd, v_forp); --trung.luu 12-03-2021 nhan tien thi subtype = 016
            IF v_ccycd = 'VND' THEN
                IF v_forp = 'F' THEN
                    V_EXRATE := 1;
                ELSE
            ...
            exception when NO_DATA_FOUND then
                /*IF NVL(p_txmsg.txnum, 'xx') LIKE '55%' THEN
                    RETURN systemnums.C_SUCCESS;
                END IF;*/
                p_err_code := '-930026';
                plog.setendsection (pkgctx, 'fn_txPreAppCheck');
            ...
            l_desc:='Overseas inward remittance (sub) dated '||to_char(TO_DATE (p_txmsg.txfields('01').value, 'dd/MM/RRRR'),'DD Mon YYYY') ;
            ...
            VALUES (TO_DATE (p_txmsg.txfields('01').value, 'dd/MM/RRRR'),p_txmsg.txnum,'N',v_feecd,NULL,v_amt,v_feeamt,v_feerate,v_vatrate,v_vatamt,SEQ_FEETRAN.NEXTVAL,l_desc,v_ccycd,NULL,'F', NULL, 'N', NULL, NULL,'016','OTHER',v_custodycd,v_feecode);
        end if;
    end if;
        -- Thoai.tran 04-Sep-2020
        -- Send mail FX
        sendmailfx(p_txmsg.txnum,'6703');
    end if;
