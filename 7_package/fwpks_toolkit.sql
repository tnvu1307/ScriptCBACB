...
FUNCTION fn_genBankRequest(p_tltxcd varchar2)
   ...
   BEGIN
    ...
    for rec in(
        SELECT OBJTYPE, OBJNAME, TRFCODE, FLDBANK, FLDACCTNO, FLDBANKACCT, FLDREFCODE,AFFECTDATE, FLDNOTES, AMTEXP,SUBCOREBANK, AUTOSENDTOBANK
             FROM CRBTXMAP MST WHERE MST.OBJTYPE = 'T' AND MST.OBJNAME = p_tltxcd AND 0=1
    )
    loop
        ...
        l_source_proc:=l_source_proc || CHR (10)
         ...
         || '           v_strNOTES:=' || pr_parse_txdescexp(rec.FLDNOTES, p_tltxcd) || ';';
       ...
       /*  || CHR (10)
             ...
             || CHR (10)
             || '                       update ddmast set balance = balance - v_strVALUE, pendinghold=pendinghold+v_strVALUE where acctno = v_strAFACCTNO;'
             || CHR (10)
             || '                       INSERT INTO DDTRAN(TXNUM,TXDATE,ACCTNO,TXCD,NAMT,CAMT,ACCTREF,DELTD,REF,AUTOID,TLTXCD,BKDATE,TRDESC)'
             || CHR (10)
             || '                       VALUES (p_txmsg.txnum, TO_DATE (p_txmsg.txdate, systemnums.C_DATE_FORMAT),v_strAFACCTNO,''0003'',v_strVALUE,NULL,v_strREFAUTOID,p_txmsg.deltd,v_strVALUE,seq_DDTRAN.NEXTVAL,p_txmsg.tltxcd,p_txmsg.busdate,v_strNOTES);'
             || CHR (10)
             || '                       INSERT INTO DDTRAN(TXNUM,TXDATE,ACCTNO,TXCD,NAMT,CAMT,ACCTREF,DELTD,REF,AUTOID,TLTXCD,BKDATE,TRDESC)'
             || CHR (10)
             || '                       VALUES (p_txmsg.txnum, TO_DATE (p_txmsg.txdate, systemnums.C_DATE_FORMAT),v_strAFACCTNO,''0013'',v_strVALUE,NULL,v_strREFAUTOID,p_txmsg.deltd,v_strVALUE,seq_DDTRAN.NEXTVAL,p_txmsg.tltxcd,p_txmsg.busdate,v_strNOTES);'
             || CHR (10)
             || '                   elsif  v_refdorc = ''C'' and v_refunhold =''Y'' then'
             || CHR (10)
             || '                       --Dr HoldBalance transfer amount'
             || CHR (10)
             || '                       update ddmast set holdbalance = holdbalance - v_strVALUE where acctno = v_strAFACCTNO;'
             || CHR (10)
             || '                       INSERT INTO DDTRAN(TXNUM,TXDATE,ACCTNO,TXCD,NAMT,CAMT,ACCTREF,DELTD,REF,AUTOID,TLTXCD,BKDATE,TRDESC)'
             || CHR (10)
             || '                       VALUES (p_txmsg.txnum, TO_DATE (p_txmsg.txdate, systemnums.C_DATE_FORMAT),v_strAFACCTNO,''0003'',v_strVALUE,NULL,v_strREFAUTOID,p_txmsg.deltd,v_strVALUE,seq_CITRAN.NEXTVAL,p_txmsg.tltxcd,p_txmsg.busdate,v_strNOTES);'
             || CHR (10)
             || '                   elsif  v_refdorc = ''C'' and v_refunhold =''N'' then'
             || CHR (10)
             || '                       --Dr HoldBalance transfer amount'
             || CHR (10)
             || '                       update ddmast set balance = balance + v_strVALUE where acctno = v_strAFACCTNO;'
             || CHR (10)
             || '                       INSERT INTO DDTRAN(TXNUM,TXDATE,ACCTNO,TXCD,NAMT,CAMT,ACCTREF,DELTD,REF,AUTOID,TLTXCD,BKDATE,TRDESC)'
             || CHR (10)
             || '                       VALUES (p_txmsg.txnum, TO_DATE (p_txmsg.txdate, systemnums.C_DATE_FORMAT),v_strAFACCTNO,''0012'',v_strVALUE,NULL,v_strREFAUTOID,p_txmsg.deltd,v_strVALUE,seq_CITRAN.NEXTVAL,p_txmsg.tltxcd,p_txmsg.busdate,v_strNOTES);'
             || CHR (10)
             || '                   end if;'*/
             || CHR (10)
         || '   end if ;'
