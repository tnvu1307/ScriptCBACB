...
/** ----------------------------------------------------------------------------------------------------
 ** Module: TX
 ** Description: Internal transfer stocks (tele)
...
plog.setbeginsection (pkgctx, 'fn_txAppAutoCheck');
   IF p_txmsg.deltd = 'N' THEN
     ...
     If txpks_check.fn_aftxmapcheck(p_txmsg.txfields('02').value,'DDMAST','02','2268')<>'TRUE' then 
         p_err_code := errnums.C_SA_TLTX_NOT_ALLOW_BY_ACCTNO;
         plog.setendsection (pkgctx, 'fn_txAppAutoCheck');
         RETURN errnums.C_BIZ_RULE_INVALID;
     End if;

     If txpks_check.fn_aftxmapcheck(p_txmsg.txfields('04').value,'DDMAST','04','2268')<>'TRUE' then 
         p_err_code := errnums.C_SA_TLTX_NOT_ALLOW_BY_ACCTNO;
         plog.setendsection (pkgctx, 'fn_txAppAutoCheck');
         RETURN errnums.C_BIZ_RULE_INVALID;
     End if;
...
BEGIN
   IF p_txmsg.deltd <> 'Y' THEN -- Normal transaction
      ...
      l_txdesc:= cspks_system.fn_DBgen_trandesc_with_format(p_txmsg,'2268','SE','0044','0003');
      ...
      l_txdesc:= cspks_system.fn_DBgen_trandesc_with_format(p_txmsg,'2268','SE','0040','0001');
      INSERT INTO SETRAN(TXNUM,TXDATE,ACCTNO,TXCD,NAMT,CAMT,ACCTREF,DELTD,REF,AUTOID,TLTXCD,BKDATE,TRDESC)
      ...
      l_txdesc:= cspks_system.fn_DBgen_trandesc_with_format(p_txmsg,'2268','SE','0043','0004');
      INSERT INTO SETRAN(TXNUM,TXDATE,ACCTNO,TXCD,NAMT,CAMT,ACCTREF,DELTD,REF,AUTOID,TLTXCD,BKDATE,TRDESC)
      ...
      l_txdesc:= cspks_system.fn_DBgen_trandesc_with_format(p_txmsg,'2268','SE','0045','0002');
      INSERT INTO SETRAN(TXNUM,TXDATE,ACCTNO,TXCD,NAMT,CAMT,ACCTREF,DELTD,REF,AUTOID,TLTXCD,BKDATE,TRDESC)
