SET DEFINE OFF;
CREATE OR REPLACE PACKAGE txpks_#8823ex
/**----------------------------------------------------------------------------------------------------
 ** Package: TXPKS_#8823EX
...
CREATE OR REPLACE PACKAGE BODY txpks_#8823ex
IS
   pkgctx   plog.log_ctx;
...
BEGIN
    ...
    ***************************************************************************************************/
    IF p_txmsg.deltd <> 'Y' THEN
        ...
        L_AUTOID := SEQ_VSTP_SETTLE_LOG.NEXTVAL;
        L_VERSION := TO_CHAR(P_TXMSG.TXDATE, 'RRMMDD') || P_TXMSG.TXNUM || '.' || TO_CHAR(L_AUTOID);

        FOR RECOD IN (
            SELECT * FROM ODMAST WHERE DELTD = 'N' AND ORDERID = L_ORDERID
        )
        LOOP
            INSERT INTO VSTP_SETTLE_LOG(AUTOID, TXNUM, TXDATE, CUSTODYCD, DDACCTNO, ORDERID, MEMBERID, AMT, ODAMT, VERSION, STATUS)
            VALUES (L_AUTOID, P_TXMSG.TXNUM, P_TXMSG.TXDATE, RECOD.CUSTODYCD, RECOD.DDACCTNO, L_ORDERID, RECOD.MEMBER, RECOD.NETAMOUNT, RECOD.EXECAMT, L_VERSION, '4');
        END LOOP;
        ...
        FOR REC IN (
            ...
            CBBONDACB.CSPKS_VSTP.PRC_CALL_2202(L_DATA, P_ERR_CODE);
