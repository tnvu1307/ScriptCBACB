SET DEFINE OFF;
CREATE OR REPLACE PACKAGE txpks_#6652ex
/**----------------------------------------------------------------------------------------------------
 ** Package: TXPKS_#6652EX
...
CREATE OR REPLACE PACKAGE BODY txpks_#6652ex
IS
   pkgctx   plog.log_ctx;
   ...
   c_feetype          CONSTANT CHAR(2) := '69';
   c_feecode          CONSTANT CHAR(2) := '70';
    ...
    IF p_txmsg.deltd <> 'Y' THEN
     ...
     if p_txmsg.txfields ('69').value = '1' then
          INSERT INTO DDTRAN(TXNUM,TXDATE,ACCTNO,TXCD,NAMT,CAMT,ACCTREF,DELTD,REF,AUTOID,TLTXCD,BKDATE,TRDESC)
                VALUES (p_txmsg.txnum, TO_DATE (p_txmsg.txdate, systemnums.C_DATE_FORMAT),p_txmsg.txfields ('06').value,'0003',ROUND(p_txmsg.txfields('11').value+p_txmsg.txfields('12').value,0),NULL,'',p_txmsg.deltd,'',seq_DDTRAN.NEXTVAL,p_txmsg.tltxcd,p_txmsg.busdate,'' || '' || '');

          INSERT INTO DDTRAN(TXNUM,TXDATE,ACCTNO,TXCD,NAMT,CAMT,ACCTREF,DELTD,REF,AUTOID,TLTXCD,BKDATE,TRDESC)
                VALUES (p_txmsg.txnum, TO_DATE (p_txmsg.txdate, systemnums.C_DATE_FORMAT),p_txmsg.txfields ('06').value,'0011',ROUND(p_txmsg.txfields('11').value+p_txmsg.txfields('12').value,0),NULL,'',p_txmsg.deltd,'',seq_DDTRAN.NEXTVAL,p_txmsg.tltxcd,p_txmsg.busdate,'' || '' || '');
    ...
    /* Formatted on 8/19/2019 3:43:04 PM (QP5 v5.126) */
     INSERT INTO crbtxreq (reqid,
      ...
      VALUES   (seq_crbtxreq.NEXTVAL,
                ...
                p_txmsg.txfields ('69').VALUE,p_txmsg.txfields ('84').VALUE,TO_DATE (p_txmsg.busdate, systemnums.c_date_format));
