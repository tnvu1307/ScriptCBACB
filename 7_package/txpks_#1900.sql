SET DEFINE OFF;
CREATE OR REPLACE PACKAGE txpks_#1900
/** ----------------------------------------------------------------------------------------------------
 ** Module: TX
 ** Description: Record pledge ticker
...
CREATE OR REPLACE PACKAGE BODY txpks_#1900
IS
   pkgctx   plog.log_ctx;
...
plog.setbeginsection (pkgctx, 'pr_txlog');
  ...
  ----xu ly cap nhat lai cfcustodycd & cffullname trong tllog doi voi GD ko qua code VB
   ...
   INSERT INTO tllogfld(AUTOID, TXNUM, TXDATE, FLDCD, NVALUE, CVALUE, TXDESC)
      VALUES( seq_tllogfld.NEXTVAL, p_txmsg.txnum, TO_DATE(p_txmsg.txdate, systemnums.C_DATE_FORMAT),'09',0,p_txmsg.txfields('09').value,'Issue code');
   plog.debug(pkgctx, 'abt to insert into tllogfld');
   INSERT INTO tllogfld(AUTOID, TXNUM, TXDATE, FLDCD, NVALUE, CVALUE, TXDESC)
...
plog.setbeginsection (pkgctx, 'fn_txAppAutoCheck');
   IF p_txmsg.deltd = 'N' THEN
     ...
     If txpks_check.fn_aftxmapcheck(p_txmsg.txfields('03').value,'SEMAST','03','1900')<>'TRUE' then
     ...
     l_AFMASTcheck_arr := txpks_check.fn_AFMASTcheck(p_txmsg.txfields('03').value,'AFMAST','CUSTODYCD');
...
*/
     ...
     IF NOT ( INSTR('A',l_STATUS) > 0) THEN
        ...
        RETURN errnums.C_BIZ_RULE_INVALID;
     END IF;
/*  TRIBUI BO DIEU KIEN CHECK SO DU CK
...
BEGIN
   IF NOT p_txmsg.deltd <> 'Y' THEN -- Normal transaction

   /*  TRIBUI C.DIEM KEU BO PHAN NAY
...
-- Run Pre Update
   IF txpks_#1900EX.fn_txPreAppUpdate(p_txmsg,p_err_code) <> systemnums.C_SUCCESS THEN
       RETURN errnums.C_BIZ_RULE_INVALID;
   END IF;
-- Run Auto Update
   ...
   END IF;
-- Run After Update
   IF txpks_#1900EX.fn_txAftAppUpdate(p_txmsg,p_err_code) <> systemnums.C_SUCCESS THEN
       RETURN errnums.C_BIZ_RULE_INVALID;
   END IF;
...
-- Run Pre check
   IF txpks_#1900EX.fn_txPreAppCheck(p_txmsg,p_err_code) <> systemnums.C_SUCCESS THEN
       RETURN errnums.C_BIZ_RULE_INVALID;
   END IF;
-- Run Auto check
   ...
   END IF;
-- Run After check
   IF txpks_#1900EX.fn_txAftAppCheck(p_txmsg,p_err_code) <> systemnums.C_SUCCESS THEN
       RETURN errnums.C_BIZ_RULE_INVALID;
   END IF;
...
BEGIN
   ...
   LOOP
       ...
       IF txpks_#1900.fn_AutoTxProcess (l_txmsg,
                                        p_err_code,
                                        p_err_param
          ) <> systemnums.c_success
       THEN
           plog.debug (pkgctx,
           'got error 1900: ' || p_err_code
           );
           ROLLBACK;
           plog.setendsection (pkgctx, 'fn_txrevert');
           RETURN errnums.C_SYSTEM_ERROR;
