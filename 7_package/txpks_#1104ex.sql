    ...
    IF p_txmsg.deltd <> 'Y' THEN
        ...
        from ESCROW es
            LEFT JOIN (select rq.reqid,rq.reqcode, rq.reqtxnum,  rq.status status
                  ...
                  ) rq on es.unholdreqid = rq.reqtxnum
        where upper(es.ESCROWID) = upper(trim(p_txmsg.txfields('01').value))
            and ( es.ddstatus in ('B') or (es.ddstatus='A' and nvl(rq.status,'C')='R') )
            and DELTD <> 'Y';
        if l_count = 0 then
            p_err_code := '-180022';
    ...
    ***************************************************************************************************/
    ...
    IF p_txmsg.deltd <> 'Y' THEN
        ...
        BEGIN
            ...
            WHERE RQ.REQTXNUM = SC.UNHOLDREQID
                AND RQ.REQCODE = 'UNHOLD1104' AND RQ.STATUS = 'R'
        ...
        BEGIN
            select holdreqid into l_holdreq from escrow where escrowid = p_txmsg.txfields('01').value;

            pck_bankapi.Bank_UNholdbalance(
                                          l_holdreq,  ---txnum cua giao dich hold
                                          p_txmsg.txfields('03').value,  --- tk ddmast
                                          TO_NUMBER(p_txmsg.txfields('10').value),  -- so tien
                                          'UNHOLD1104', --request code cua nghiep vu trong allcode
        ...
        update ESCROW
        set blockedamt = blockedamt + nvl(l_moveamt,0) - TO_NUMBER(p_txmsg.txfields('10').value),
            unholdreqid = l_reqid,
            pstatus = pstatus||'|'||status, status = decode(status,'B','BS','A'),
            ddstatus = 'A',
    ...
    ELSE --Revert trans
        ...
        update ESCROW
        set qtty = qtty - TO_NUMBER(p_txmsg.txfields('10').value),
            blockedqtty = blockedqtty + TO_NUMBER(p_txmsg.txfields('10').value),
            pstatus = pstatus||'|'||status, status = decode(status,'BS','B','BC'),
            ddstatus = 'B',
            last_change = CURRENT_TIMESTAMP
        where ESCROWID = p_txmsg.txfields('01').value and deltd = 'N';
