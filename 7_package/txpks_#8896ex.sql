SET DEFINE OFF;
CREATE OR REPLACE PACKAGE txpks_#8896ex
/**----------------------------------------------------------------------------------------------------
 ** Package: TXPKS_#8896EX
...
CREATE OR REPLACE PACKAGE BODY txpks_#8896ex
IS
   pkgctx   plog.log_ctx;
...
BEGIN
    ...
    ***************************************************************************************************/
    IF p_txmsg.deltd = 'N' THEN
        ...
        loop
             If txpks_check.fn_aftxmapcheck(r.ACCTNO,'SEMAST','22','8896')<>'TRUE' then
...
BEGIN
    ...
    ***************************************************************************************************/
    ...
    IF p_txmsg.txfields('25').value = 'NB' THEN
        IF p_txmsg.deltd <> 'Y' THEN -- Normal transaction
            ...
            loop
               ...
               ;
                 ...
                 UPDATE ETFWSAP
                 SET
                   --QTTY = QTTY - (r.qtty), TRUNG.LUU: 08-03-2021 SHBVNEX-2134 KHONG TRU QTTY KHI THANH TOAN
       ...
       ELSE -- Reversal
            ...
            UPDATE TLLOG
             SET DELTD = 'Y'
              ...
              for r in (select (od.afacctno || et.codeid) ACCTNO, et.qtty,et.codeid from odmast od, etfwsap et where od.orderid = et.orderid and od.orderid = p_txmsg.txfields('22').value)
                loop
                    select trade into v_trade from semast where acctno = r.ACCTNO;
                    if v_trade < R.QTTY THEN
                        p_err_code := '-200502';
    ...
    ELSE -- type = 1
        ...
        loop
            ...
            /*INSERT INTO SETRAN(TXNUM,TXDATE,ACCTNO,TXCD,NAMT,CAMT,ACCTREF,DELTD,REF,AUTOID,TLTXCD,BKDATE,TRDESC)
            VALUES (p_txmsg.txnum, TO_DATE (p_txmsg.txdate, systemnums.C_DATE_FORMAT),r.ACCTNO,'0011',ROUND(r.qtty,0),NULL,r.ACCTNO,p_txmsg.deltd,r.ACCTNO,seq_SETRAN.NEXTVAL,p_txmsg.tltxcd,p_txmsg.busdate,'' || '' || '');
*/
            UPDATE SEMAST
             SET
