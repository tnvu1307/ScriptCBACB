SET DEFINE OFF;
CREATE OR REPLACE PACKAGE txpks_#2305ex
/**----------------------------------------------------------------------------------------------------
 ** Package: TXPKS_#2305EX
...
CREATE OR REPLACE PACKAGE BODY txpks_#2305ex
IS
   pkgctx   plog.log_ctx;
...
BEGIN
    ...
    ***************************************************************************************************/
    IF P_TXMSG.DELTD <> 'Y' THEN
        SELECT COUNT(1) INTO L_COUNT FROM SEDEPOSIT_TPRL WHERE AUTOID = TO_NUMBER(P_TXMSG.TXFIELDS('99').VALUE) AND STATUS IN ('S') AND DELTD = 'N';
        IF L_COUNT = 0 THEN
            p_err_code := '-100165';
            plog.setendsection (pkgctx, 'fn_txPreAppCheck');
            RETURN errnums.C_BIZ_RULE_INVALID;
...
BEGIN
    ...
    ***************************************************************************************************/
    IF P_TXMSG.DELTD <> 'Y' THEN
        ...
        VALUES (p_txmsg.txnum, TO_DATE (p_txmsg.txdate, systemnums.C_DATE_FORMAT),p_txmsg.txfields ('03').value,'0050',p_txmsg.txfields('10').value,NULL,p_txmsg.txfields('99').value,p_txmsg.deltd,p_txmsg.txfields('99').value,seq_SETRAN.NEXTVAL,p_txmsg.tltxcd,p_txmsg.busdate,'' || '' || '');
        ...
        UPDATE SEDEPOSIT_TPRL SET STATUS = 'R' WHERE AUTOID = TO_NUMBER(P_TXMSG.TXFIELDS('99').VALUE);
    END IF;
