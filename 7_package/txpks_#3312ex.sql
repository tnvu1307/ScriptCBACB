SET DEFINE OFF;
CREATE OR REPLACE PACKAGE txpks_#3312ex
/**----------------------------------------------------------------------------------------------------
 ** Package: TXPKS_#3312EX
...
CREATE OR REPLACE PACKAGE BODY txpks_#3312ex
IS
   pkgctx   plog.log_ctx;
...
BEGIN
    ...
    ***************************************************************************************************/
    ...
    IF p_txmsg.deltd <> 'Y' THEN -- NORMAL TRANSACTION
        ...
        -- Xoa du lieu cu.
        Delete from t_caschd_list where camastid = l_camastid;
        Delete from caschdtemp_list where camastid = l_camastid;
               ...
               --NGAY 04/02/2020 NAMTV THEM SU KIEN 033 TUONG TU 016
                ...
                IF l_catype = '014' THEN
                    l_sql := 'INSERT INTO caschdtemp_list (AUTOID, CAMASTID, AFACCTNO, CODEID, EXCODEID, BALANCE, QTTY, AMT, AQTTY, AAMT, STATUS,REQTTY,REAQTTY,RETAILBAL,PBALANCE, PQTTY,PAAMT,TRADE,ROUND)  '
                              ...
                              || ' ,TRUNC((SUM(MST.TRADE + MST.MARGIN + MST.WTRADE + MST.MORTAGE +MST.BLOCKWITHDRAW + MST.EMKQTTY + MST.BLOCKDTOCLOSE + MST.BLOCKED + MST.SECURED + MST.REPO + MST.NETTING + MST.DTOCLOSE + MST.WITHDRAW + MST.HOLD) - SUM(TR.AMOUNT))*'||l_right_exrate||'/'|| l_left_exrate||') RETAILBAL,'
                              || '  TRUNC((SUM(MST.TRADE + MST.MARGIN + MST.WTRADE + MST.MORTAGE +MST.BLOCKWITHDRAW + MST.EMKQTTY + MST.BLOCKDTOCLOSE + MST.BLOCKED + MST.SECURED + MST.REPO + MST.NETTING + MST.DTOCLOSE + MST.WITHDRAW + MST.HOLD) - SUM(TR.AMOUNT))*'||l_right_exrate||'/'||l_left_exrate||') PBALANCE, '
                              || nvl(l_qttyexp,'''''') || '  PQTTY,  ROUND(' || nvl(l_aamtexp,'''''') || ',0) PAAMT,'
                              || ' TRUNC((SUM(MST.TRADE + MST.MARGIN + MST.WTRADE + MST.MORTAGE +MST.BLOCKWITHDRAW + MST.EMKQTTY + MST.BLOCKDTOCLOSE + MST.BLOCKED + MST.SECURED + MST.REPO + MST.NETTING + MST.DTOCLOSE + MST.WITHDRAW + MST.HOLD) - SUM(TR.AMOUNT))) TRADE, '|| l_round ||' ROUND'
                ...
                ELSIF l_catype = '022' THEN
                    l_sql := 'INSERT INTO caschdtemp_list (AUTOID, CAMASTID, AFACCTNO, CODEID, EXCODEID, BALANCE, QTTY, AMT, AQTTY, AAMT, STATUS,REQTTY,REAQTTY,RETAILBAL,TRADE,RQTTY,round)  '
                             ...
                             || ' (SUM(MST.TRADE + MST.MARGIN + MST.WTRADE + MST.MORTAGE +MST.BLOCKWITHDRAW + MST.EMKQTTY + MST.BLOCKDTOCLOSE + MST.BLOCKED + MST.SECURED + MST.REPO + MST.NETTING + MST.DTOCLOSE + MST.WITHDRAW + MST.HOLD) - SUM(TR.AMOUNT)) BALANCE, '
                             ...
                             || ' , 0  RETAILBAL,  '
                              || ' TRUNC((SUM(MST.TRADE + MST.MARGIN + MST.WTRADE + MST.MORTAGE +MST.BLOCKWITHDRAW + MST.EMKQTTY + MST.BLOCKDTOCLOSE + MST.BLOCKED + MST.SECURED + MST.REPO + MST.NETTING + MST.DTOCLOSE + MST.WITHDRAW + MST.HOLD) - SUM(TR.AMOUNT))) TRADE, ROUND(' || nvl(l_rqttyexp,'''''') || ',0) RQTTY, '|| l_round ||' ROUND'
                             || ' FROM SBSECURITIES SYM, CAMAST CA, SEMAST MST,  '
                             || ' (SELECT MST.ACCTNO, NVL(DTL.AMT,0) AMOUNT FROM SEMAST MST LEFT JOIN '
                             || ' (select DTL.ACCTNO, sum(DTL.AMT) amt From '
                             || ' (SELECT TR.ACCTNO, SUM((CASE WHEN TX.TXTYPE=''D'' THEN -TR.NAMT WHEN TX.TXTYPE=''C'' THEN TR.NAMT ELSE 0 END)) AMT  '
                ...
                ELSIF l_catype = '017' THEN
                  ...
                  ELSIF l_catype = '023' THEN
                    l_sql := 'INSERT INTO caschdtemp_list (AUTOID, CAMASTID, AFACCTNO, CODEID, EXCODEID, BALANCE, QTTY, AMT, AQTTY, AAMT, STATUS,REQTTY,REAQTTY,RETAILBAL,TRADE,ROUND,PQTTY,INTAMT)  '
                             ...
                             || ' , 0  RETAILBAL , '
                             || ' TRUNC((SUM(MST.TRADE + MST.MARGIN + MST.WTRADE + MST.MORTAGE +MST.BLOCKWITHDRAW + MST.EMKQTTY + MST.BLOCKDTOCLOSE + MST.BLOCKED + MST.SECURED + MST.REPO + MST.NETTING + MST.DTOCLOSE + MST.WITHDRAW + MST.HOLD) - SUM(TR.AMOUNT))) TRADE, '|| l_round ||' ROUND, ' || nvl(l_qttyexp,'''''') || ' PQTTY , '
                ...
                ELSE
                    l_sql := 'INSERT INTO caschdtemp_list (AUTOID, CAMASTID, AFACCTNO, CODEID, EXCODEID, BALANCE, QTTY, AMT, AQTTY, AAMT, STATUS,REQTTY,REAQTTY,RETAILBAL,TRADE,INTAMT,RQTTY,ROUND)  '
                             ...
                             || nvl(l_qttyexp,'''''') || '  QTTY, ROUND(' || nvl(l_amtexp,'''''') || ',0) AMT, ' || nvl(l_aqttyexp,'''''') || ' AQTTY, ROUND(' || nvl(l_aamtexp,'''''') || ',0) AAMT, ''A'' STATUS,' || nvl(l_reqttyexp,'''''') || '  REQTTY,' || nvl(l_reaqttyexp,'''''') || '  REAQTTY '
                             || ' , 0  RETAILBAL,  '
                             || ' TRUNC((SUM(MST.TRADE + MST.MARGIN + MST.WTRADE + MST.MORTAGE +MST.BLOCKWITHDRAW + MST.EMKQTTY + MST.BLOCKDTOCLOSE + MST.BLOCKED + MST.SECURED + MST.REPO + MST.NETTING + MST.DTOCLOSE + MST.WITHDRAW + MST.HOLD) - SUM(TR.AMOUNT))) TRADE, ROUND(' || nvl(l_intamtexp,'''''') || ',0) INTAMT, ROUND(' || nvl(l_rqttyexp,'''''') || ',0) RQTTY, '|| l_round ||' ROUND'
                             || ' FROM SBSECURITIES SYM, CAMAST CA, SEMAST MST,  '
                             || ' (SELECT MST.ACCTNO, NVL(DTL.AMT,0) AMOUNT FROM SEMAST MST LEFT JOIN '
                             || ' (select DTL.ACCTNO, sum(DTL.AMT) amt From '
                             || ' (SELECT TR.ACCTNO, SUM((CASE WHEN TX.TXTYPE=''D'' THEN -TR.NAMT WHEN TX.TXTYPE=''C'' THEN TR.NAMT ELSE 0 END)) AMT  '
                ...
                insert into t_caschd_list (AUTOID, CAMASTID, AFACCTNO, CODEID, EXCODEID, BALANCE, QTTY, AMT, AQTTY, AAMT, STATUS,REQTTY,REAQTTY,RETAILBAL,TRADE,PBALANCE,PQTTY,PAAMT,INTAMT,RQTTY)
                    ...
                    -- Tinh gia tri chung khoan cho quyen ve.
                              ...
                              SELECT COUNT(*) INTO l_count_temp FROM CASCHDTEMP_LIST,afmast af
                                  ...
                                  if(l_count_temp>0  )    THEN
                                        INSERT INTO t_caschd_list (AUTOID, CAMASTID, AFACCTNO, CODEID, EXCODEID, BALANCE, QTTY, AMT, AQTTY, AAMT, STATUS,REQTTY,REAQTTY,RETAILBAL,TRADE,PBALANCE,PQTTY,PAAMT,INTAMT,RQTTY,ORGPBALANCE)
                                       ...
                                       (case when l_catype in ('014','023') then 0 else (l_dbl_qttyexp-nvl(schdsum.qtty,0)) end) QTTY, --(CASE WHEN (temp.qtty=0 ) THEN temp.qtty ELSE (l_dbl_qttyexp-nvl(schdsum.qtty,0))END) QTTY,
                                        ...
                                        --End 31/05/2018 DieuNDA: Bo case WHEN (temp.qtty=0 ) THEN temp.qtty chi lay nhanh ELSE (l_dbl_qttyexp-nvl(schdsum.qtty,0))
                                                  FROM (select * from (SELECT temp.* FROM CASCHDTEMP_LIST temp,afmast
                                                        ...
                                                        FROM t_caschd_list , afmast
                                                        WHERE camastid=l_camastid AND afmast.acctno=t_caschd_list.afacctno
        ...
        END IF;
    Begin
        -- Cap nhat lai thong tin nhung TK cu khi co thay doi thong tin SKQ.
        If p_txmsg.txfields('21').value = 'U' then
            For rec22 in (
                Select * from t_caschd_list where camastid=l_camastid
            )loop
                Update caschd_list
                    set
                        BALANCE = rec22.BALANCE,
                        QTTY = rec22.QTTY,
                        AMT =rec22.AMT,
                        AQTTY = rec22.AQTTY,
                        AAMT = rec22.AAMT,
                        ...
                        TRADE = rec22.TRADE,
                        ISRECEIVE = 'N'
                    where camastid=rec22.camastid and afacctno=rec22.afacctno;
            End Loop;
        End if;
        ...
        -- Cho bo sung
         INSERT INTO caschd_list (AUTOID,CAMASTID,BALANCE,QTTY,AMT,AQTTY,AAMT,STATUS,AFACCTNO,CODEID,EXCODEID,DELTD,PSTATUS,REFCAMASTID,
                        RETAILSHARE,DEPOSIT,REQTTY,REAQTTY,RETAILBAL,PBALANCE,PQTTY,PAAMT,COREBANK,ISCISE,DFQTTY,ISCI,ISSE,ISRO,
                        TQTTY,TRADE,INBALANCE,OUTBALANCE,PITRATEMETHOD,ISEXEC,NMQTTY,DFAMT,INTAMT,RQTTY,RORETAILBAL,SENDPBALANCE,
                        SENDQTTY,SENDAQTTY,SENDAMT,
                        ISRECEIVE,ACTIONTYPE,
                        INQTTY,CUTPBALANCE,CUTQTTY,CUTAQTTY,CUTAMT,PAFACCTNO,ORGPBALANCE,
                        TRFFEE,VSDSTATUS,QTTYCANCEL,SWIFTCLIENT, TXDATE,lastchange)
         select AUTOID,CAMASTID,BALANCE,QTTY,AMT,AQTTY,AAMT,STATUS,AFACCTNO,CODEID,EXCODEID,'N' DELTD,PSTATUS,REFCAMASTID,
                        RETAILSHARE,DEPOSIT,REQTTY,REAQTTY,RETAILBAL,PBALANCE,PQTTY,PAAMT,COREBANK,ISCISE,DFQTTY,ISCI,ISSE,ISRO,
                        TQTTY,TRADE,INBALANCE,OUTBALANCE,PITRATEMETHOD,ISEXEC,NMQTTY,DFAMT,INTAMT,RQTTY,RORETAILBAL,SENDPBALANCE,
                        SENDQTTY,SENDAQTTY,SENDAMT,
                        'N' ISRECEIVE, 'N' ACTIONTYPE,
                        INQTTY,CUTPBALANCE,CUTQTTY,CUTAQTTY,CUTAMT,PAFACCTNO,ORGPBALANCE,
                        TRFFEE,VSDSTATUS,QTTYCANCEL,SWIFTCLIENT, l_currdate, SYSTIMESTAMP
         from t_caschd_list news where not EXISTS (select * from caschd_list olds where news.camastid=olds.camastid and news.afacctno=olds.afacctno);
    End;
