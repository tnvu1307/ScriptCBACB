...
BEGIN
    ...
    ***************************************************************************************************/
    if p_txmsg.deltd <> 'Y' then
        ...
        BEGIN
          SELECT afacctno INTO l_afacctno FROM catransfer tr, caschd sc
          WHERE tr.caschdid = sc.autoid AND tr.autoid = TO_NUMBER(p_txmsg.txfields('01').value);
          nmpks_ems.pr_gentemplateEM30(p_txmsg.txfields(c_camastid).value,
                                       l_afacctno);
        EXCEPTION
          WHEN OTHERS THEN
            plog.error(pkgctx, SQLERRM || dbms_utility.format_error_backtrace);
