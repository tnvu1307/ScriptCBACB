SET DEFINE OFF;
CREATE OR REPLACE PACKAGE TXPKS_#2259EX
/**----------------------------------------------------------------------------------------------------
 ** Package: TXPKS_#2259EX
...
CREATE OR REPLACE PACKAGE BODY TXPKS_#2259EX
IS
   pkgctx   plog.log_ctx;
   ...
   c_blockqtty        CONSTANT CHAR(2) := '12';
...
BEGIN
    ...
    ***************************************************************************************************/
    ...
    select WITHDRAW into v_WithdrawQtty
    from semast
    where acctno = p_txmsg.txfields('03').value;

    if v_WithdrawQtty < TO_NUMBER(p_txmsg.txfields('10').value)  then
        p_err_code := -200502;
        plog.setendsection (pkgctx, 'fn_txPreAppCheck');
        RETURN errnums.C_BIZ_RULE_INVALID;
    end if;
