SET DEFINE OFF;
CREATE OR REPLACE PACKAGE txpks_#6698ex
/**----------------------------------------------------------------------------------------------------
 ** Package: TXPKS_#6698EX
...
CREATE OR REPLACE PACKAGE BODY txpks_#6698ex
IS
   pkgctx   plog.log_ctx;
    ...
    ***************************************************************************************************/
    ...
    IF p_txmsg.deltd <> 'Y' THEN -- Normal transaction
        ...
        if not p_txmsg.txfields('05').value is null then
            pr_gen_buf_dd_member(p_txmsg.txfields('04').value ,p_txmsg.txfields(c_memberid).value,0,-(ROUND(p_txmsg.txfields('10').value,0)),0,0,0,0) ;
        end if;

        L_REQTXNUM := P_TXMSG.TXFIELDS ('95').VALUE;
        /*
        SELECT COUNT(1) INTO L_COUNT FROM STSCHD_NETOFF WHERE TO_CHAR(AUTOID) = L_REQTXNUM;
        IF L_COUNT > 0 THEN
            FOR REC IN (
        ...
        END IF;
        */
        SELECT COUNT(1) INTO L_COUNT FROM ODMAST WHERE ORDERID = L_REQTXNUM;
        IF L_COUNT > 0 THEN
            UPDATE ODMAST SET ISHOLD = 'N', LASTCHANGE = SYSTIMESTAMP WHERE ORDERID = L_REQTXNUM;
        END IF;

        IF L_REQTXNUM LIKE 'VSTP.%' THEN
            UPDATE VSTP_SETTLE_LOG
            SET STATUS = '3.2', PSTATUS = STATUS, LASTCHANGE = SYSTIMESTAMP
            WHERE VERSION = SUBSTR(L_REQTXNUM, INSTR(L_REQTXNUM, '.') + 1);
        END IF;

        --thunt18/01/2020:them gui mail khi khong du so du tien
        nmpks_ems.pr_gentemplatecashsec(p_txmsg.txfields('04').value,'',p_txmsg.txfields('05').value,(ROUND(p_txmsg.txfields('10').value,0)),'');
