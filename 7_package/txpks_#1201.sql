SET DEFINE OFF;
CREATE OR REPLACE PACKAGE txpks_#1201
/** ----------------------------------------------------------------------------------------------------
 ** Module: TX
 ** Description: Collect outstanding fee
...
CREATE OR REPLACE PACKAGE BODY txpks_#1201
IS
   pkgctx   plog.log_ctx;
...
BEGIN
   IF p_txmsg.deltd <> 'Y' THEN -- Normal transaction
      ...
      l_txdesc:= cspks_system.fn_DBgen_trandesc_with_format(p_txmsg,'1201','DD','0003','0001');
      ...
      l_txdesc:= cspks_system.fn_DBgen_trandesc_with_format(p_txmsg,'1201','DD','0088','0001');
      INSERT INTO DDTRAN(TXNUM,TXDATE,ACCTNO,TXCD,NAMT,CAMT,ACCTREF,DELTD,REF,AUTOID,TLTXCD,BKDATE,TRDESC)
...
-- Run Pre Update
   IF txpks_#1201EX.fn_txPreAppUpdate(p_txmsg,p_err_code) <> systemnums.C_SUCCESS THEN
...
-- Run Auto Update
   ...
   END IF;
-- Run After Update
   IF txpks_#1201EX.fn_txAftAppUpdate(p_txmsg,p_err_code) <> systemnums.C_SUCCESS THEN
       RETURN errnums.C_BIZ_RULE_INVALID;
   END IF;
...
-- Run Pre check
   IF txpks_#1201EX.fn_txPreAppCheck(p_txmsg,p_err_code) <> systemnums.C_SUCCESS THEN
       RETURN errnums.C_BIZ_RULE_INVALID;
   END IF;
-- Run Auto check
   ...
   END IF;
-- Run After check
   IF txpks_#1201EX.fn_txAftAppCheck(p_txmsg,p_err_code) <> systemnums.C_SUCCESS THEN
       RETURN errnums.C_BIZ_RULE_INVALID;
   END IF;
