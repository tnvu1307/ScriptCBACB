SET DEFINE OFF;
CREATE OR REPLACE PACKAGE txpks_#6699ex
/**----------------------------------------------------------------------------------------------------
 ** Package: TXPKS_#6699EX
...
CREATE OR REPLACE PACKAGE BODY txpks_#6699ex
IS
   pkgctx   plog.log_ctx;
...
BEGIN
    ...
    */
    ...
    IF L_COUNT > 0 THEN
        UPDATE ODMAST SET ISHOLD = 'Y', LASTCHANGE = SYSTIMESTAMP WHERE ORDERID = L_REQTXNUM;
    END IF;

    L_REQTXNUM := P_TXMSG.TXFIELDS ('95').VALUE;
    IF L_REQTXNUM LIKE 'VSTP.%' THEN
        UPDATE VSTP_SETTLE_LOG
        SET STATUS = '1.2', PSTATUS = STATUS, LASTCHANGE = SYSTIMESTAMP
