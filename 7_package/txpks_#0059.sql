    ...
    for r in (select *From cfmast cf where   instr((select fn_get_custodycd_fr_motherfund(p_txmsg.txfields('88').value) from dual),cf.custodycd) >0)
        loop
          ...
          INSERT INTO AFTRAN(TXNUM,TXDATE,ACCTNO,TXCD,NAMT,CAMT,ACCTREF,DELTD,REF,AUTOID,TLTXCD,BKDATE,TRDESC)
                VALUES (p_txmsg.txnum, TO_DATE (p_txmsg.txdate, systemnums.C_DATE_FORMAT),r.custid,'0131',0,NULL,r.custid,p_txmsg.deltd,r.custid,seq_AFTRAN.NEXTVAL,p_txmsg.tltxcd,p_txmsg.busdate,'' || '' || '');

          INSERT INTO AFTRAN(TXNUM,TXDATE,ACCTNO,TXCD,NAMT,CAMT,ACCTREF,DELTD,REF,AUTOID,TLTXCD,BKDATE,TRDESC)
                VALUES (p_txmsg.txnum, TO_DATE (p_txmsg.txdate, systemnums.C_DATE_FORMAT),r.custid,'0002',0,'N',r.custid,p_txmsg.deltd,r.custid,seq_AFTRAN.NEXTVAL,p_txmsg.tltxcd,p_txmsg.busdate,'' || '' || '');



          UPDATE CFMAST
             SET
               CFCLSDATE = TO_DATE(p_txmsg.txdate, systemnums.C_DATE_FORMAT),
