...
BEGIN
    ...
    ***************************************************************************************************/
    IF p_txmsg.deltd <> 'Y' THEN -- Normal transaction
        --8866 Nhan tien ban chung khoan
        l_tltxcd       := '8866';
        ...
        ------VAT
        l_txmsg.txfields ('12').defname   := 'VAT';
        l_txmsg.txfields ('12').TYPE      := 'N';
        l_txmsg.txfields ('12').VALUE      := p_txmsg.txfields('27').value;
        ...
        IF L_COUNT > 0 THEN
            FOR REC IN (
                ...
                BEGIN
                    IF txpks_#8866.fn_batchtxprocess (l_txmsg, p_err_code, l_err_param) <> systemnums.c_success THEN
