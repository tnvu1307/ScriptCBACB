...
BEGIN
    ...
    ***************************************************************************************************/
    IF p_txmsg.deltd <> 'Y' THEN -- Normal transaction
        ...
        BEGIN
            SELECT HOLD_SE_TEMP INTO HOLD_TEMP FROM ESCROW WHERE ESCROWID = P_TXMSG.TXFIELDS('92').VALUE AND NVL(DELTD, 'N') = 'N';
        ...
        if p_txmsg.txfields('10').value > hold_temp then
            p_err_code := '-930109';
            plog.setendsection (pkgctx, 'fn_txPreAppCheck');
            RETURN errnums.C_BIZ_RULE_INVALID;
        else
            INSERT INTO SETRAN(TXNUM,TXDATE,ACCTNO,TXCD,NAMT,CAMT,ACCTREF,DELTD,REF,AUTOID,TLTXCD,BKDATE,TRDESC)
                 VALUES (p_txmsg.txnum, TO_DATE (p_txmsg.txdate, systemnums.C_DATE_FORMAT),v_seacctno,'0107',p_txmsg.txfields('10').value,NULL,'',p_txmsg.deltd,'',seq_SETRAN.NEXTVAL,p_txmsg.tltxcd,TO_DATE(p_txmsg.txdate,'dd/MM/RRRR'),'' || '' || '');
             Update semast
            ...
            SET HOLD_SE = HOLD_SE - P_TXMSG.TXFIELDS('10').VALUE
            WHERE ESCROWID = P_TXMSG.TXFIELDS('92').VALUE
            AND UNHOLD ='N'
            AND AUTOID = P_TXMSG.TXFIELDS('11').VALUE
            AND NVL(DELTD, 'N') = 'N';

            -- Thoai.tran 03/08/2021
            -- SHBVNEX-1310 Hold nhieu lan nen check khi hold_temp = 0 thi unhold
                if hold_temp = 0 then
                    UPDATE ESCROW_HOLD_TEMP
                    SET UNHOLD = 'Y'
                    WHERE ESCROWID = P_TXMSG.TXFIELDS('92').VALUE
