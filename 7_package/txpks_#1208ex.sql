SET DEFINE OFF;
CREATE OR REPLACE PACKAGE txpks_#1208ex
/**----------------------------------------------------------------------------------------------------
 ** Package: TXPKS_#1208EX
...
CREATE OR REPLACE PACKAGE BODY txpks_#1208ex
IS
   pkgctx   plog.log_ctx;
...
BEGIN
    ...
    ***************************************************************************************************/
    IF p_txmsg.deltd <> 'Y' THEN
        ...
        FROM
        (
            SELECT * FROM FEE_BOOKING_RESULT WHERE STATUS = 'S' AND DELTD = 'N'
            UNION ALL
            SELECT * FROM FEE_BOOKING_RESULTHIST WHERE STATUS = 'S' AND DELTD = 'N'
        ) DT
...
IS
    ...
    L_TRN_DT VARCHAR2(50);
    L_REFTXNUM VARCHAR2(50);
    L_REFTXDATE DATE;
BEGIN
    ...
    ***************************************************************************************************/
    IF p_txmsg.deltd <> 'Y' THEN
        ...
        SELECT GLOBALID, TRN_DT, TXNUM, TXDATE
        INTO L_GLOBALID, L_TRN_DT, L_REFTXNUM, L_REFTXDATE
        FROM SETTLE_FEE_LOG
        WHERE TO_CHAR(FEEBOOKINGAUOID) = L_AUTOID
        AND STATUS = 'C';
        ...
        INSERT INTO CRBTXREQ (REQID, OBJTYPE, OBJNAME, TRFCODE, REQCODE,
                              ...
                              CURRENCY, TOCURRENCY, FEETYPE, EXCHANGERATE, DORC,
                              DESBANKACCOUNT, TRN_DT, REFTXNUM, REFTXDATE)
        VALUES (L_REQID, 'T', '1208', 'CANCELSETTLEFEE', 'CANCELSETTLEFEE',
                    L_GLOBALID, p_txmsg.txnum, TO_DATE (p_txmsg.txdate, systemnums.c_date_format), TO_DATE (p_txmsg.txdate, systemnums.c_date_format), 'SHV',
                    L_REFCASAACCT, L_DDACCTNO, L_TXAMT, 'P', L_NOTE,
                    L_CURRENCY, L_CURRENCY, 'TTM', '1', L_SETTLETYPE,
                    L_SPRACNO, L_TRN_DT, L_REFTXNUM, L_REFTXDATE);

        UPDATE SETTLE_FEE_LOG SET STATUS = 'D' WHERE TO_CHAR(FEEBOOKINGAUOID) = L_AUTOID;
    END IF;
