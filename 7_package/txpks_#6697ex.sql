SET DEFINE OFF;
CREATE OR REPLACE PACKAGE txpks_#6697ex
/**----------------------------------------------------------------------------------------------------
 ** Package: TXPKS_#6697EX
...
CREATE OR REPLACE PACKAGE BODY txpks_#6697ex
IS
   pkgctx   plog.log_ctx;
...
BEGIN
    ...
    ***************************************************************************************************/
    ...
    if not p_txmsg.txfields('05').value is null then
        pr_gen_buf_dd_member(p_txmsg.txfields('04').value ,p_txmsg.txfields(c_memberid).value,0,0,-(ROUND(p_txmsg.txfields('10').value,0)),0,0,p_txmsg.txfields('10').value) ;
    end if;

    L_REQTXNUM := P_TXMSG.TXFIELDS ('95').VALUE;
    IF L_REQTXNUM LIKE 'VSTP.%' THEN
        UPDATE VSTP_SETTLE_LOG
        SET UNHOLD = UNHOLD + ROUND(P_TXMSG.TXFIELDS('10').VALUE, 0),
            STATUS = (CASE WHEN UNHOLD + ROUND(P_TXMSG.TXFIELDS('10').VALUE, 0) = BEFOR_HOLD THEN '1.1' ELSE STATUS END),
            PSTATUS = STATUS,
