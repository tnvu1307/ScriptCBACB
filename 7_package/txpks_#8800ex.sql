    ...
    ***************************************************************************************************/
    IF p_txmsg.deltd <> 'Y' THEN -- Reversal transaction
        ...
        if p_txmsg.TXSTATUS  = TXSTATUSNUMS.c_txcompleted then
			...
			/*
            If p_txmsg.txfields('16').value = 'I072' then
                CSPKS_ODPROC.PR_AUTO_8894(p_txmsg.txfields('15').value,p_txmsg.tlid, 'O',P_ERR_CODE);
                if P_ERR_CODE <> systemnums.C_SUCCESS then
            ...
            End if;
            -- Online
            If p_txmsg.txfields('16').value = 'I069' and p_txmsg.tlid = systemnums.C_ONLINE_USERID then
                cspks_filemaster.PR_FILLTER_TBLI069_4web(p_txmsg.txfields('15').value,'A', p_txmsg.tlid,p_err_code, p_err_message);
                if P_ERR_CODE <> systemnums.C_SUCCESS then
            ...
            -- From CB
            ElsIf p_txmsg.txfields('16').value = 'I069' and p_txmsg.tlid <> systemnums.C_ONLINE_USERID then
                cspks_filemaster.PR_FILLTER_TBLI069(p_txmsg.txfields('15').value,p_txmsg.tlid,p_err_code, p_err_message);
                if P_ERR_CODE <> systemnums.C_SUCCESS then
