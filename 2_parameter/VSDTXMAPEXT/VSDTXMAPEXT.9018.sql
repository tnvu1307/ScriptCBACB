...
from camast where camastid in (select camastid  from CASCHD where autoid = ''<$FILTERID>'')', '', '', '', '', NULL);Insert into VSDTXMAPEXT   (OBJTYPE, OBJNAME, TRFCODE, FLDNAME, FLDTYPE, AMTEXP, CMDSQL, CAPTION, EN_CAPTION, CHSTATUS, SPLIT, ODRNUM) Values   ('T', '9018', '564.CORP.REPE.TIEN', 'DESCRIPTION', 'C', '@VSD confirmed', '', 'Diễn giải', 'Description', 'C', '', 1233219999);Insert into VSDTXMAPEXT   (OBJTYPE, OBJNAME, TRFCODE, FLDNAME, FLDTYPE, AMTEXP, CMDSQL, CAPTION, EN_CAPTION, CHSTATUS, SPLIT, ODRNUM) Values   ('T', '9018', '564.CORP.REPE.TIEN', 'CICCODE', 'C', '<$TXNUM>', 'SELECT CF.SWIFTCODE
...
)', 'Số tiền đã trừ thuế cổ tức', 'Net amount', 'C', 'N', 13);Insert into VSDTXMAPEXT   (OBJTYPE, OBJNAME, TRFCODE, FLDNAME, FLDTYPE, AMTEXP, CMDSQL, CAPTION, EN_CAPTION, CHSTATUS, SPLIT, ODRNUM) Values   ('T', '9018', '564.CORP.REPE.TIEN', 'CST_BANK_ACCT', 'C', '<$TXNUM>', 'select NVL(refcasaacct,'''')REFCA from ddmast where  accounttype = ''IICA'' and status <> ''C'' and afacctno in (select afacctno from caschd where autoid = ''<$FILTERID>'')', 'TK nhận tiền cổ tức ', 'Account number', 'C', '', 1233219999);Insert into VSDTXMAPEXT   (OBJTYPE, OBJNAME, TRFCODE, FLDNAME, FLDTYPE, AMTEXP, CMDSQL, CAPTION, EN_CAPTION, CHSTATUS, SPLIT, ODRNUM) Values   ('T', '9018', '564.CORP.REPE.TIEN', 'ACT_DT', 'C', '<$TXNUM>', 'select TO_CHAR(actiondate,''RRRRMMDD'') from CAMAST WHERE CAMASTID IN (SELECT CAMASTID FROM caschd WHERE AUTOID = ''<$FILTERID>'')', 'Ngày thanh toán', 'Payment date', '', '', 1233219999);Insert into VSDTXMAPEXT   (OBJTYPE, OBJNAME, TRFCODE, FLDNAME, FLDTYPE, AMTEXP, CMDSQL, CAPTION, EN_CAPTION, CHSTATUS, SPLIT, ODRNUM) Values   ('T', '9018', '564.CORP.REPE.TIEN', 'CRLFPAYD_DT', 'C', '<$TXNUM>', 'select  case when instr(''005,028,006'',catype) >0  then ''||:98A::MEET//''||to_char(actiondate,''RRRRMMDD'')
            when instr(''005,028,006'',catype) <= 0  then ''||:98A::RDTE//''||to_char(reportdate,''RRRRMMDD'')
             else  '''' end
from camast where camastid in (select camastid  from CASCHD where autoid = ''<$FILTERID>'')', '', '', '', '', 1233219999);Insert into VSDTXMAPEXT   (OBJTYPE, OBJNAME, TRFCODE, FLDNAME, FLDTYPE, AMTEXP, CMDSQL, CAPTION, EN_CAPTION, CHSTATUS, SPLIT, ODRNUM) Values   ('T', '9018', '564.CORP.REPE.TIEN', 'ELIG_TYPE', 'C', '<$TXNUM>', 'SELECT (CASE WHEN INSTR(''001,002,008,011'',SB.SECTYPE)>0 THEN ''UNIT/''|| REPLACE(TO_CHAR(NVL(CAS.TRADE,0),''FM999999999999990.999999''),''.'','','')
...
FROM(
    ...
    WHERE CAMASTID IN (SELECT CAMASTID FROM <$REFOBJ> WHERE AUTOID = ''<$FILTERID>'')
       ...
       END) VAL
FROM CAMAST
WHERE CAMASTID IN (SELECT CAMASTID  FROM CASCHD WHERE AUTOID = ''<$FILTERID>'')', 'Số tiền trả cho 1 TP', '', 'C', 'N', 17);Insert into VSDTXMAPEXT   (OBJTYPE, OBJNAME, TRFCODE, FLDNAME, FLDTYPE, AMTEXP, CMDSQL, CAPTION, EN_CAPTION, CHSTATUS, SPLIT, ODRNUM) Values   ('T', '9018', '564.CORP.REPE.TIEN', 'CRLFPAYMENT_PERCENT', 'C', '<$TXNUM>', 'SELECT REPLACE(
