...
(SELECT  ACCTNO,ACCLIMIT FROM USERAFLIMIT WHERE  TLIDUSER=''<$TELLERID>'' AND  TYPERECEIVE=''MR'') USF
...
AND AF.ACCTNO NOT IN (SELECT MAF.AFACCTNO
                FROM ODPROBRKAF MAF,ODPROBRKMST MST 
                WHERE MAF.REFAUTOID=MST.AUTOID and MAF.DELTD <> ''Y'' and af.ACCTNO=MAF.AFACCTNO
                AND MST.PROBRKMSTTYPE = (SELECT PROBRKMSTTYPE FROM ODPROBRKMST WHERE AUTOID=''<@KEYVALUE>'')
) )a
LEFT JOIN
(Select NVL(US.ACCTLIMIT,0) LIMITMAX,NVL(US.ALLOCATELIMMIT-US.USEDLIMMIT,0) USERHAVE
  from userlimit US where US.tliduser=''<$TELLERID>'') us on 0=0', 'AFMAST', 'frmAFMAST', NULL, 'EXEC', NULL, 5000, 'N', 1, NULL, 'Y', 'T', NULL, 'N', NULL);COMMIT;