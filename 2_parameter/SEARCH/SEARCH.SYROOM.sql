SET DEFINE OFF;DELETE FROM SEARCH WHERE 1 = 1 AND NVL(SEARCHCODE,'NULL') = NVL('SYROOM','NULL');Insert into SEARCH   (SEARCHCODE, SEARCHTITLE, EN_SEARCHTITLE, SEARCHCMDSQL, OBJNAME, FRMNAME, ORDERBYCMDSQL, TLTXCD, CNTRECORD, ROWPERPAGE, AUTOSEARCH, INTERVAL, AUTHCODE, ROWLIMIT, CMDTYPE, CONDDEFFLD, BANKINQ, BANKACCT) Values   ('SYROOM', 'Tra cứu Room CK hệ thống', 'System room Master', 'SELECT ROWNUM NO, CODEID, SYMBOL, LISTINGQTTY, ROOMLIMIT, UBROOMLIMIT, UBROOMUSED, UBROOMREMAIN, SYROOMLIMIT, SYROOMUSED, SYROOMREMAIN,
      DB_ROOMLIMIT, DB_ROOMUSED, DB_ROOMREMAIN, MRPRICELOAN, MRMARKETVAL, TCMARKETVAL, MRBALANCE, TCBALANCE, 
  ...
  FROM (SELECT  SB.CODEID, SB.SYMBOL, SB.LISTINGQTTY, (SB.LISTINGQTTY*SYSR1.VARVALUE/100) ROOMLIMIT, 
          RM.SYROOMLIMIT, nvl(AFPR.PRINUSED,0) SYROOMUSED, 
          RM.SYROOMLIMIT - nvl(AFPR.PRINUSED,0) SYROOMREMAIN,
          RM.DB_ROOMLIMIT, nvl(AFPR.DBPRINUSED,0) DB_ROOMUSED, 
          RM.DB_ROOMLIMIT - nvl(AFPR.DBPRINUSED,0) DB_ROOMREMAIN, sb.basicprice MRPRICELOAN, NVL(LN.MRSEAMT_MARKET,0) MRMARKETVAL, NVL(LN.DPSEAMT_MARKET,0) TCMARKETVAL,
          RM.ROOMLIMIT UBROOMLIMIT,
          nvl(AFPR.UBPRINUSED,0) UBROOMUSED, 
          RM.ROOMLIMIT - nvl(AFPR.UBPRINUSED,0) UBROOMREMAIN,
          ...
          FROM VW_MARGINROOMSYSTEM RM, SECURITIES_INFO SB, SECURITIES_RISK SRK, SYSVAR SYSR, ALLCODE A0,SYSVAR SYSR1,
                 (SELECT CODEID, 
                         ...
                         SUM(DECODE(RESTYPE, ''M'',PRINUSED, 0)) UBPRINUSED,
                         SUM(DECODE(RESTYPE, ''O'',PRINUSED, 0)) DBPRINUSED
                         FROM VW_AFPRALLOC_ALL WHERE RESTYPE in (''S'',''M'',''O'') GROUP BY CODEID) AFPR,
                 (
