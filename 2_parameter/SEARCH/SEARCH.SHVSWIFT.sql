SET DEFINE OFF;DELETE FROM SEARCH WHERE 1 = 1 AND NVL(SEARCHCODE,'NULL') = NVL('SHVSWIFT','NULL');Insert into SEARCH   (SEARCHCODE, SEARCHTITLE, EN_SEARCHTITLE, SEARCHCMDSQL, OBJNAME, FRMNAME, ORDERBYCMDSQL, TLTXCD, CNTRECORD, ROWPERPAGE, AUTOSEARCH, INTERVAL, AUTHCODE, ROWLIMIT, CMDTYPE, CONDDEFFLD, BANKINQ, BANKACCT) Values   ('SHVSWIFT', 'Điện Swift nhận từ khách hàng', 'Management of Swift messages from Client', 'SELECT S.MSGID REQUESTID,SDTLD.SETTLEDATE,SDTLD.TRADEDATE, MTTYPE,V.<@CDCONTENT> DESCS, CF.CUSTODYCD, SENDERBICCODE, S.CFNSTATUS, S.STATUS, A.<@CDCONTENT> CFNSTATUSTEXT,
...
FROM SWIFTMSGMAPLOG S, (SELECT *FROM CFMAST WHERE STATUS <> ''C'' ) CF,
    (SELECT * FROM SWIFTMSGMAPLOGDTL WHERE DEFNAME IN (''FIAC.97A.SAFE'',''USECU.97A.SAFE'')) SDTL,
    (
        SELECT MSGID,
...
AND NVL(S.SI,''N'') = A2.CDVAL (+)', 'SHVSWIFT', 'SHVSWIFT', ' CREATEDATEORDER DESC, CREATETIME DESC', '1710', 0, 5000, 'Y', 30, NULL, 'Y', 'T', NULL, 'N', NULL);COMMIT;