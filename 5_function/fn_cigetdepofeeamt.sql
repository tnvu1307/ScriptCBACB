...
BEGIN
  ...
  SELECT nvl (DEPOLASTDT,TO_DATE(strBUSDATE,'DD/MM/RRRR')) into v_dtmLASTACRDT FROM DDMAST WHERE AFACCTNO=strACCTNO and isdefault='Y';

  SELECT TO_DATE(strBUSDATE,'DD/MM/RRRR')- v_dtmLASTACRDT INTO v_dblNUMOFDAYS FROM DUAL;

  IF v_dblNUMOFDAYS > 0 THEN
    RETURN 0; --khong co phi den han
  ELSE
